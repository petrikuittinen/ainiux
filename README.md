...
Chat images are kept as raw, content-addressed files under `~/.ainiux/media/sha256/`; SQLite stores only their SHA-256 references and metadata. `[media] expiration_days = 7` controls explicit TUI `/cleanup`, while `[media] auto_expiration_days = 30` controls the automatic cleanup run when chat mode starts. Set either value to `0` to disable that cleanup path. Explicit cleanup protects the currently open thread. When media expires—or a managed file is found missing—the affected saved thread remains readable but is marked `[RO]` and cannot be continued or edited.
...
The TUI also runs `/insert`, `/attach`, `/fetch`, `/search`, and managed-media cleanup through cancellable runtime jobs. `/insert FILE_OR_URL` places text in the chat input at its cursor; `/attach PATH` prepares provider context and persists supported images with the thread. `/cleanup` expires managed media unused past `[media] expiration_days`, except media referenced by the currently open thread. `/help` toggles a persistent, scrollable command panel that is not sent to the provider or saved.
