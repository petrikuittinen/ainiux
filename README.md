...
ainiux image --provider xai -p "a red cube on white" --size 1k --ar 1:1 --quality low
ainiux video --provider xai -m grok-imagine-video-1.5 -p "a red cube rotating" --duration 1 --resolution 480p
ainiux video --provider gemini -m gemini-omni-1.1-flash -p "a red cube rotating" --resolution 360p
```

Text, Markdown, HTML, PDF, DOCX, XLSX, PPTX, CSV, and JSON can be attached with `--attach`. Local CSV and JSON stay native text; fetched CSV/JSON become Markdown. Ainiux's PDF↔Markdown conversion is deliberately **super fast**: it is a local, single-process C++17 reader/writer with bounded streaming, no Python runtime or external PDF service, and fast enough to process thousands of pages per second on typical text PDFs. Ainiux also has fast local DOCX, XLSX, and PPTX conversion using the already-linked zlib and shared bounded OOXML reader. PPTX uses canonical Markdown: top-level `---` lines outside fenced code separate slides, the first heading is the title, and a final `> **Speaker notes:**` quote block stores notes. It preserves slide order, dimensions, text formatting, lists, links, tables, notes, and supported embedded image bytes; charts, SmartArt, media, unsupported shapes, themes, animation, and exact layout become warnings/placeholders. `--input deck.pptx --output deck.md` writes deduplicated image bytes under `deck.media/`; stdout, chat, TUI, editor-open, and browser conversion use visible image-omission placeholders instead. PDF, DOCX, XLSX, PPTX, and HTML compound conversions pass through canonical Markdown, so presentation images are intentionally lost when the destination is PDF, DOCX, or XLSX. CJK, Hebrew, and Arabic use a subsetted system or `--font` TrueType for PDF output. `--encoding` converts UTF-16, Windows-1250/1251/1252, ISO-8859-1/2, and KOI8-R/U locally; CJK names use `iconv` when installed. The editor asks when a file is not valid UTF-8. PNG, JPEG, and GIF input is available through compatible Chat Completions models. URL fetching happens only when explicitly requested with `--fetch-url`, `/fetch`, `/attach URL`, `/insert URL`, or the agent fetch tool; remote PDF, DOCX, XLSX, PPTX, CSV, and JSON are converted locally to Markdown. A URL inside a prompt never triggers a fetch. Private, loopback, link-local, multicast, and metadata addresses are blocked unless explicitly allowed.
...
| `fal` | `fal_ai` | `FAL_API_KEY`, `FAL_KEY` (image/video generation; not a chat profile) |
...
The Video tab reads the effective layered `videos.conf`, renders each model's
scalar controls, accepts the catalog-supported image/video/audio references,
and submits a cancellable server job. Successful MP4 output is played with the
browser's HTML5 video player and can be downloaded from an authenticated
artifact route; provider credentials and provider output URLs stay server-side.
...
curl -H "Authorization: Bearer $AINIUX_SERVER_SECRET" \
  ...
  -d '{"provider":"openai","model":"MODEL","messages":[{"role":"user","content":"Hello"}]}' \
...
```sh
ainiux server --bind 192.0.2.10 --tls-cert /secure/server.crt \
  --tls-key /secure/server.key --workspace .
```

Remote sessions cannot request or inherit Yolo unless the server also starts
with `--allow-remote-yolo`. See [the control API guide](docs/api.md) for routes,
limits, authentication, certificate, Host, and Origin rules.
Run `scripts/test-control-server.sh --build` for the current authenticated
control-server smoke check. `scripts/test-control-server-pr3.sh` retains the
broader live mock-provider, idempotency, SSE, and cancellation check.
...
Ainiux does not yet implement an OpenAI-compatible `/v1` proxy, REPL/TUI image or video generation, `/loop`, sub-agents, or a native Anthropic Messages adapter. CLI PDF/DOCX/XLSX/PPTX conversion is implemented; compound document conversion goes through Markdown. Office conversion is normalized rather than lossless: macros, themes, comments, exact layout, and rich application-specific objects are not reproduced. TUI `/export` and `/export-last` write JSON, PDF, DOCX, or Markdown; latest-message export also supports one worksheet per Markdown table in XLSX and a sole Markdown table as RFC 4180 CSV. The WebUI exports JSON, PDF, DOCX, or Markdown. Whole-transcript XLSX/PPTX and dedicated browser transcript XLSX/PPTX downloads remain deferred. CJK, Hebrew, and Arabic in Markdown→PDF use a subsetted system or `--font` TrueType; encrypted PDFs remain later work. The embedded browser controller, one-shot control-API jobs including config-driven image generation, edits, and video generation, interactive agent sessions, revision-safe chat threads, revision-safe workspace review/dired/file mutations and editor assist, TLS/direct non-loopback policy, the MCP server adapter, and CLI image/video generation are implemented. Image models use `images.conf`; video models use `videos.conf` with the `fal_queue`, `replicate_predictions`, `xai_imagine`, `gemini_interactions`, and `gemini_veo` adapters; keys remain server-side. The terminal UI uses native POSIX `termios` or Win32 console mode ownership with shared ANSI/VT parsing rather than ncurses. HTML extraction is intentionally lightweight: it does not execute JavaScript or implement a browser DOM.
