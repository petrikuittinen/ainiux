...
Interactive agent starts in Act. `/plan` switches subsequent turns to the trusted planning prompt and updates the input title to `project plan`; `/act` restores full coding tools. Switches preserve transcript and compaction state, are idempotent, and are rejected while a job is active. `/new` and every fresh agent entry start in Act with `smart` permissions. Plan keeps read/search/index and configured web research, uses inspection-only commands, and may create or edit only root `PLANS.md`, `PLAN.md`, `TODO.md`, `AGENTS.md`, or case-sensitive `*.md` files below an existing `docs/plans/` tree. It cannot create directories, delete files, rebuild the index, or write source/README files.

Mutation tools prefer project-relative paths. An absolute path that resolves inside the active project is normalized before containment and mode-policy checks. Interactive Agent persists `/permissions confirm|smart|yolo` in project `agent.sqlite`: `confirm` asks for native writes and outside-project native access; `smart` allows project writes and native access under the canonical system-temp directory but asks elsewhere; `yolo` allows validated native access and model commands. Confirm and Smart ask for every model-issued `run_command`. Guard hard denials and Plan write restrictions remain non-elevatable in every mode. The prompt border shows the active permission at the right.

Exact-path `list_directory`, `read_file`, `read_many`, `edit_file`, `write_file`, `str_replace`, `create_directory`, `rename_path`, and `remove` can operate outside the project when policy allows. External structured edits retain UTF-8, size, hash, atomic-write, cancellation, symlink, and redaction checks, but create no project history/index entry; `replace_symbol` stays project-only. `run_command` remains direct argv execution with a fixed PATH—no shell wrappers, pipes, redirection, substitutions, or arbitrary executable paths—and can use an approved canonical external cwd or explicit absolute external operand. Headless Ask decisions are denied and never restore interactive Yolo.

For OpenRouter and DeepSeek on their official base URLs, interactive Agent also retrieves the API key's remaining credit in a cancellable background request. The prompt border shows it immediately after the permission mode, for example `smart 4.50 USD`; DeepSeek currency codes such as `CNY` are preserved, and multiple returned balances are separated with `·`. The label refreshes at startup, after provider changes, and after completed turns. Unsupported providers, custom base URLs, unlimited/null OpenRouter key limits, missing credentials, and failed balance lookups leave the credit label empty without blocking Agent use.
