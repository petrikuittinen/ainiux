...
Foreground refreshes immediately report discovery, changed-file scanning, graph resolution, and snapshot commit progress on `stderr`; terminals reuse one line and redirected output receives bounded phase/progress lines.
...
Python, C, and C++ additionally record approximate `call`, `import`, `include`, `inherit`, and `instantiate` references. A project-wide lightweight resolver keeps unresolved and ambiguous rows explicit, computes distinct caller counts, and uses deterministic PageRank only as a secondary ranking signal. It handles common direct calls, simple `self`/`this` and inferred receiver cases, prototypes versus definitions, and ordinary inheritance; dynamic dispatch, reflection, macros, overload resolution, and unusual declarations remain verification cases rather than compiler-grade facts.
...
The report also includes a compact graph table for total, resolved, ambiguous, and unresolved references. Every printed symbol carries its stored distinct caller count and fixed-precision PageRank; individual reference rows remain omitted.
...
The switch is session-scoped and does not probe, load, create, refresh, or mutate `index.sqlite`. Exact-path live reads, directory listing, guarded commands, Git, ordinary edits, and configured network tools remain available. Index-backed overview/glob/search/symbol/caller/test tools and `replace_symbol` are hidden, hints are omitted, and any existing database is untouched. Interactive Agent asks before creating a missing index; No/Esc starts with indexing off for the current process. Completed indexes refresh without prompting, while corrupt indexes produce an explicit rebuild error.
...
**Read / search / index (agent and security-review where noted):** `project_overview`, `list_directory`, `glob`, `search_text`/`grep`/`find`, `search_symbol`, `get_skeleton`, `read_symbol`, `find_callers`, `find_callees`, `read_file`, `read_many`, `run_command` (allowlisted; no shell), `git_status`, `git_diff` (bounded read-only git CLI), `index_status`, `index_update`, `find_tests`, `inspect_code_task`.
