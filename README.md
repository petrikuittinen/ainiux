...
The switch is session-scoped and does not probe, load, create, refresh, or mutate `index.sqlite` during startup. `glob`, `search_text`, `grep`, and `find` remain available through a cancellable live source-tree traversal with the same schemas, filters, ordering, and bounds as their indexed forms. Exact-path reads, directory listing, guarded commands, Git, ordinary edits, and configured network tools also remain available. Index overview/symbol/test/status tools and `replace_symbol` are hidden, and any existing database is untouched until the user explicitly runs `/index-code`. Interactive Agent asks before creating a missing index; Enter/Y creates it, while N/Esc continues unindexed. Creation progress stays live on the status line, then the language/files/lines/indexed/skipped/symbols totals table from the start of `--print-index` is appended to Agent history. `/index-code` later creates and enables a skipped index without restarting the session. Completed indexes refresh without prompting. Headless Run and Plan refresh an existing index but do not create a missing one. Optional index failures warn and continue with live filesystem tools; cancellation still cancels startup.
