...
v0.99 introduced `--security-review`, a headless read-only whole-project workflow on the project index and native provider function calls. v1.01 added one-shot agent mode; v1.02 enabled ordinary workspace writes; v1.03 separates user profile (`~/.ainiux/`) from project agent/index state (`.ainiux-pr/`), adds multi-turn project sessions, compact tool lines, and window-% auto-compact; v1.04 streams tool activity live, shows elapsed seconds, and keeps chat/agent transcripts isolated. Interactive Guard Ask y/n approvals (with `approvals` persistence) are landed for `--agent`. Entry points: one-shot `ainiux run` / `--run` / `-r` and interactive `ainiux agent` / `--agent` / `-a`. Security review remains read-only. Later v1.0 work includes plan/security agent modes and call-graph tools.
