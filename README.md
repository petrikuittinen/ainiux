...
**v1.07** — active development. Core surfaces are usable daily: scriptable CLI, REPL, full-screen chat TUI, AI editor, multi-provider chat, durable image and canonical-Markdown attachments, safe URL fetch, web search hooks, document conversion, concurrent benchmarks, judge grading, headless whole-project security review, one-shot Act and Plan agents (`ainiux run`, `ainiux plan`, `--run`, `--plan`, and file forms), and an interactive agent TUI (`ainiux agent` / `--agent` / `-a`) with session-scoped `/plan` and `/act`, project-local `.ainiux-pr/` state, live compact tool activity, chat/agent transcript isolation, ordinary Act workspace edits, code-enforced planning-document-only writes in Plan, and interactive Guard Ask approvals. User chat library remains `~/.ainiux/ainiux.db`. Dedicated security/refactor agent modes remain later roadmap work.
...
Agent mode uses trusted master + Act coding or Plan prompts plus a static protocol appendix (native tools when the provider supports function calling, otherwise the XML `<tool_call>` channel). When present, workspace-root `AGENTS.md` is loaded (capped, UTF-8 only) and injected as separate project-instruction context under the trusted prompt's precedence rules. It reuses the security-review read/search/inspect tools and enables mode-scoped mutations and network tools:
...
Interactive agent starts in Act. `/plan` switches subsequent turns to the trusted planning prompt and updates the input title to `project plan`; `/act` restores full coding tools. Switches preserve transcript and compaction state, are idempotent, and are rejected while a job is active. `/new` and every fresh agent entry start in Act. Plan keeps read/search/index and configured web research, uses inspection-only commands, and may create or edit only root `PLANS.md`, `PLAN.md`, `TODO.md`, `AGENTS.md`, or case-sensitive `*.md` files below an existing `docs/plans/` tree. It cannot create directories, delete files, rebuild the index, or write source/README files.

Mutation tools prefer project-relative paths. An absolute path that resolves lexically inside the active project is normalized to its project-relative form before containment and mode-policy checks. In interactive Act mode only, `read_file` and `write_file` may request one-shot **Yes/No** Guard approval for an exact resolved path outside the active project. Selecting No or Esc leaves it untouched; headless modes deny the request. External writes have no project history backup and do not update the project index. Plan mode never permits external writes.

Ordinary writes stay inside the workspace, refuse `.ainiux-pr`/`.ainiux`/`.git`/symlink escapes, keep at most one pre-overwrite copy per path under `.ainiux-pr/history/` (size/TTL configurable), and update the in-memory index snapshot so later `read_file` calls in the same run see the new hashes. The only outside-project exception is a single explicitly approved `write_file` call as described above. Project agent transcript and non-secret provider/model/request preferences are stored in `.ainiux-pr/agent.sqlite` (one thread per project; not the chat library); credentials and authorization headers are not stored there. Compact tool lines print on stderr for `--run` and in the agent TUI transcript; each completed row ends in `in N ms`, measured for that tool alone with interactive Guard approval wait excluded. The final `Task complete in X.XX seconds.` remains whole-turn wall time. In interactive agent mode, `/compact` immediately reduces only the model-visible request context while preserving the complete SQLite transcript; repeated use without enough new history is a no-op. `/new [PATH]` initializes and switches to a fresh project, resolving relative paths from the active project root and creating only a missing final component. If the target already has `.ainiux-pr`, Ainiux asks before permanently removing that project history, index, approvals, and logs; other target-directory files are left untouched. Interactive agent prompts **Yes/No** for Guard Ask actions (outside-project `read_file`/`write_file`, destructive git, recursive rm, database `remove`); outcomes are stored in `agent.sqlite` `approvals`. No/Esc is the safe default. Headless `run` and `plan` deny Ask. Still later: `find_callers`/`find_callees` (need call-graph refs) and security/refactor agent modes.

Loop limits, transport retries, history hygiene, and identical-call guards come from the shared agent loop. Status and the live diagnostic path print on `stderr`; the final assistant answer is the only normal `stdout` content. Logs default under `.ainiux-pr/logs/agent/` with the same live-flush / finalize behavior as security-review logs (`tail -f` the printed `.partial` path). Disable with `--no-agent-log`.
