...
This installs the `ainiux` binary, configuration templates, themes, editor-command prompts, and benchmark files. Existing administrator config under `/etc/xdg/ainiux/` is preserved. During an upgrade, the installer rewrites legacy boolean spellings only for known boolean fields in preserved `config.conf` and `models.conf` files; custom settings, comments, and reasoning vocabulary remain unchanged.
...
**v1.14** — active development. Core surfaces are usable daily: scriptable CLI, REPL, full-screen chat TUI, AI editor, multi-provider chat, durable image and canonical-Markdown attachments, safe URL fetch, web search hooks, document conversion, concurrent benchmarks, judge grading, headless whole-project security review, one-shot Act and Plan agents (`ainiux run`, `ainiux plan`, `--run`, `--plan`, and file forms), and an interactive agent TUI (`ainiux agent` / `--agent` / `-a`) with session-scoped `/plan` and `/act`, project-local `.ainiux-pr/` state, in-place compact tool activity, provider-supplied reasoning previews, three-strategy transcript-preserving context compaction, retained row-diff terminal rendering, punctuation-aware Markdown highlighting, chat/agent transcript isolation, ordinary Act workspace edits, code-enforced planning-document-only writes in Plan, and interactive Guard Ask approvals. The optional code index is a fast definitions-only navigation aid with static symbol importance. Display-only notices and thinking previews never enter provider context. User chat library remains `~/.ainiux/ainiux.db`. Dedicated security/refactor agent modes remain later roadmap work.

Under the hood: libcurl HTTP/SSE, cancellable runtime jobs, Chat Completions plus text-only Responses API support, a layered model capability catalog with unified reasoning controls, SQLite-backed TUI threads, retained row-diff terminal rendering, JSON chat import/export, multi-language syntax highlighting, grapheme-aware editing, and layered TOML-alike configuration.
...
`/theme` with no argument reports `off` plus available themes. `/theme off` disables palette and syntax styling while retaining layout/control sequences; selecting a named theme re-enables colors. This state follows chat ↔ editor ↔ agent switches. `--nocolors`, `[tui] colors = off`, and `[tui] theme = off` also start without styling.
...
Set the startup default with `highlight = on|off` under `[tui]`. `--nocolors` suppresses syntax colors while selection remains visible. Existing custom themes remain valid when syntax keys are omitted; accessible colors are derived from their existing semantic colors.
...
Foreground refreshes immediately report discovery, changed-file scanning, snapshot commit, and migration compaction progress on `stderr`; terminals reuse one line and redirected output receives bounded phase/progress lines.

`--index-code` recursively discovers eligible source files and creates or incrementally refreshes `.ainiux-pr/index.sqlite`. Dot-prefixed directories are hidden by POSIX convention and skipped as complete subtrees, so `.ainiux-pr`, `.git`, editor state, caches, and other hidden-directory contents never enter the code index; ordinary root dotfiles such as `.gitignore` remain available to the ignore-rule loader. It reuses the editor's case-insensitive filename detection, including JavaScript `.js`/`.mjs`/`.cjs`/`.jsx` and TypeScript `.ts`/`.mts`/`.cts`/`.tsx`. Parsing uses fast regex-assisted lexical scanners. Multi-file work uses `floor(online cores × 0.80)` workers, bounded by the number of work items; a single-file scan runs inline. It intentionally favors speed over compiler-grade accuracy. It records ordinary classes, namespaces/packages/modules, functions, methods, types, aliases, fields/properties, globals, and constants. Document and configuration scanners record useful structural symbols such as Markdown headings, markup declarations, JSON keys, SQL objects, and TOML/YAML/INI sections and keys. The web scanners also record TypeScript interfaces and type aliases, CSS selectors, at-rules, keyframes and custom properties, HTML elements with IDs and custom elements, and symbols inside HTML `<script>` and `<style>` blocks. HTML-only does not scan embedded JavaScript or CSS, and data-only JSON/import-map script blocks are skipped.

Each definition receives a cheap static importance score from its declaration kind, visibility, and scope. Lexical relevance remains primary in symbol and task searches; importance only orders otherwise comparable matches. The index stores definitions only—no references, call graph, evidence rows, caller counts, or graph scores.
...
Run a single agent goal against the current project (refreshes an existing code index, uses tools, writes final text to `stdout`):
...
For strict indexed-versus-unindexed comparisons, add `--disable-indexing` to Agent, Run, or Plan:

```sh
./ainiux run --provider openai -m MODEL --run "add unit tests" --disable-indexing
./ainiux plan "design the change" --provider openai -m MODEL --disable-indexing
./ainiux --agent --provider openai -m MODEL --disable-indexing
```

The switch is session-scoped and does not probe, load, create, refresh, or mutate `index.sqlite` during startup. `glob`, `search_text`, `grep`, and `find` remain available through a cancellable live source-tree traversal with the same schemas, filters, ordering, and bounds as their indexed forms. Exact-path reads, directory listing, guarded commands, Git, ordinary edits, and configured network tools also remain available. Index overview/symbol/test/status tools and `replace_symbol` are hidden, and any existing database is untouched until the user explicitly runs `/index-code`. Without this switch, Agent performs only a cheap read-only probe during preparation. A missing or corrupt index never blocks readiness and is never created automatically. In interactive agent mode, when no completed index is found, Agent offers a Yes/No prompt to build one (multi-threaded `refresh`); declining leaves live filesystem tools and `/index-code` available later. The input frame top-right chrome shows `index` between the permission mode and credit balance when indexing is enabled. Headless Run and Plan never prompt; they warn on stderr and continue with live tools. A completed index is attached as lazy SQLite hints, then refreshed in the background after Agent is ready. Agent opens short-lived read-only queries instead of loading the full index into memory. Optional index failures warn and continue with live tools.
...
**Read / search / index (agent and security-review where noted):** `project_overview`, `list_directory`, `glob`, `search_text`/`grep`/`find`, `search_symbol`, `get_skeleton`, `read_symbol`, `read_many`, `read_file`, `run_command` (allowlisted; no shell), `git_status`, `git_diff` (bounded read-only git CLI), `index_status`, `index_update`, `find_tests`, `inspect_code_task`.

`read_many` is the preferred reader whenever two or more paths or ranges are already known, even for providers that support parallel native tool calls. It reads 1–100 independently bounded, line-numbered ranges in one model tool call; each item defaults to the same 64 KiB limit as `read_file`, under an aggregate 256 KiB cap. `read_file` remains the single-target fallback. Agent activity shows the batch count and leading paths. To exercise it interactively, try: `Read README.md and AGENTS.md using one read_many tool call, then report the first heading from each.` The completed activity row should begin with `read_many(2 reads: ...)`.
...
Interactive agent starts in Act. `/attach PATH` queues text or PNG/JPEG/GIF images and immediately shows a success/failure notice in history (not only the status line). With a vision-capable Chat Completions model, user `/attach` images are request-local for that user turn (all tool rounds), then dropped—not stored in `.ainiux-pr/` or `~/.ainiux/media`; re-attach after `/compact` if needed. The agent tool `attach_image` loads one project image for subsequent model rounds of the same turn only (default cap 4 per turn; no automatic attach of every path mentioned in the prompt). `/plan` appends a Plan control for subsequent turns and updates the input title to `project plan`; `/act` appends an Act control and restores full mutation authority. `/goal <condition>` sets one project-scoped completion condition (persisted in `.ainiux-pr/agent.sqlite` settings): the runtime injects a short ACTIVE GOAL control, auto-continues after tool-less final answers until the model calls `goal_met` with non-empty evidence, stalls after two tool-less finals, hits the goal turn cap, or the user cancels. `/goal` shows status; `/goal clear|stop|off|cancel`, `/goal pause`, and `/goal resume` manage the goal without rewriting the trusted system prompt. `/index-code` creates and enables an index after startup indexing was declined or disabled; if one is already active, it incrementally refreshes it. `/show-index` requires an enabled index, performs the same cancellable incremental refresh, and appends only the current compact totals table to Agent history; these report rows never enter model context. Both commands show index progress. Act and Plan advertise an identical native-tool superset for cache stability, while the runtime policy remains authoritative: Plan keeps read/search/index and configured web research, permits only conservatively vetted read-only commands, and may create or edit only root `PLANS.md`, `PLAN.md`, `TODO.md`, `AGENTS.md`, or case-sensitive `*.md` files below an existing `docs/plans/` tree. It cannot create directories, delete or rename files, rebuild the index, or write source/README files. Repeating the active mode is a no-op; switches preserve transcript and compaction state and are rejected while a job is active. `/new` and every fresh agent entry start in Act with `smart` permissions.
...
The Smart/Plan read-only classifier checks the complete argv form, not just the executable name. Its project-inspection set covers `pwd`, `ls`, `cat`, `head`, non-following `tail`, `stat`, `file`, `wc`, `du`, `grep`, `rg`, print-only `find`, `diff`, `cmp`, `readlink`, and MD5/SHA/BLAKE2/`cksum` checksum display. Passive snapshots cover `ps`, `df`, `whoami`, `id`, `groups`, `who`, `uname`, `lsb_release`, `uptime`, `free`, `nproc`, and `arch`; display-only forms of `hostname`, `date`, `ifconfig`, and `ip … show|list`, plus `command -v NAME` executable lookup, are also recognized. Ordinary display/count/context/format flags and project-contained auxiliary input files are supported. Unknown options fall back to approval in Act/Smart and are denied in Plan. Symlink-following recursion, `tail --follow`, executing or file-writing `find` actions, `rg --pre`, `file --compile`, checksum verification files, output-file options, and mutating display-command forms are not vetted. `ping`, `top`, generic Git commands, interpreters, builds, and tests are intentionally excluded; use the dedicated no-prompt Git status/diff tools where applicable.

Exact-path `list_directory`, `read_file`, `read_many`, `edit_file`, `write_file`, `str_replace`, `create_directory`, `rename_path`, and `remove` use the live filesystem in Agent mode: safe project paths do not need to be present in the code index, and they can operate outside the project when policy allows. `glob` and text search use indexed candidates when indexing is enabled and equivalent live source discovery when it is off; symbol, patch, and dedicated Git tools retain their project/index scope. External structured edits retain UTF-8, size, hash, atomic-write, cancellation, symlink, and redaction checks, but create no project history/index entry; `replace_symbol` stays project-only. `run_command` remains direct `execve` argv execution with a fixed system PATH, null stdin, bounded output, and timeout/cancellation. System tools use bare names (`command -v NAME` checks the fixed PATH). Project scripts are first-class in Act: `./server.sh start`, bare `server.sh` under the project cwd/root, and `bash server.sh …` / `sh ./script.sh` (script-file form). Free-form `bash -c` / `sh -c` is denied in Confirm/Smart. Pipes, redirects, unquoted substitutions, and control operators remain structurally rejected (not interpreted); quoted literal `$` formats stay ordinary argv data. An approved command can use a canonical external cwd or explicit absolute external operand. Headless Ask decisions are denied. Interactive **Yolo** skips elevatable prompts **and** hard Guard denials (sudo, free-form shells, package managers, …) at the user's risk; Confirm/Smart still hard-deny those. Privileged commands intentionally typed by the user can also use the separate `/shell` interface.
...
For OpenRouter, OpenAI, and DeepSeek on their official base URLs, interactive Agent also retrieves the account's remaining credit in a cancellable background request. The prompt border shows it immediately after the permission mode, for example `smart 4.50 USD`; DeepSeek currency codes such as `CNY` are preserved, and multiple returned balances are separated with `·`. OpenRouter remaining credit is its purchased credit minus usage, while OpenAI uses `dashboard/billing/credit_grants.total_available`. OpenAI project-scoped keys may not be authorized for that dashboard endpoint. The label refreshes at startup, after provider changes, and after completed turns. Unsupported providers, custom base URLs, missing credentials, unauthorized keys, and failed balance lookups leave the credit label empty without blocking Agent use.


Project-scoped writes refuse `.ainiux-pr`/`.ainiux`/`.git`/symlink escapes, keep at most one pre-overwrite copy per path under `.ainiux-pr/history/` (size/TTL configurable), and update the in-memory index snapshot so later `read_file` calls in the same run see the new hashes. Touched source paths are coalesced through a cancellable background index worker; index queries consume the newest completed snapshot, potentially mutating commands request an incremental full-tree freshness check, and every successfully completed task waits for a final pass. A full-tree pass rediscovers eligible paths but reparses only added or metadata-changed files; unchanged entries and symbols are reused. Approved external changes do not receive project-history updates. Project agent transcript and non-secret provider/model/request preferences are stored in `.ainiux-pr/agent.sqlite` (one thread per project; not the chat library); credentials and authorization headers are not stored there. Each headless `run` or `plan` starts with a fresh model conversation and does not inject that stored transcript, while interactive agent mode resumes it. Compact tool lines print on stderr for `--run` and in the agent TUI transcript; each completed row ends in `in N ms`, measured for that tool alone with interactive Guard approval wait excluded. A one-shot run ends with a stable `Agent metrics: tool calls N (F failed), input …` stderr line plus output tokens and agent-goal wall time including workspace preparation and finalization. A call succeeds only when its normalized top-level result envelope contains Boolean `ok: true`; provider/transport failures remain separate. Provider token counts are preferred, with per-round estimates labeled when usage metadata is unavailable. `--quiet` suppresses this status line. The final `Task complete in X.XX seconds.` remains whole-turn wall time. In interactive agent mode, `/compact` immediately reduces only the model-visible request context while preserving the complete SQLite transcript; repeated use without enough new history is a no-op. `/new [PATH]` initializes and switches to a fresh project, resolving relative paths from the active project root and creating only a missing final component. If the target already has `.ainiux-pr`, Ainiux asks before permanently removing that project history, index, approvals, and logs; other target-directory files are left untouched. Interactive agent prompts **Yes/No** for actions selected by the active permission mode or Guard; outcomes are stored in `agent.sqlite` `approvals`. Selecting No ends that agent turn without fallback tool attempts, and a later user message starts with a fresh failure budget. No/Esc is the safe default. Headless `run` and `plan` deny Ask. Security/refactor-specific agent modes remain later work.

Loop limits, transport retries, history hygiene, and identical-call guards come from the shared agent loop. Status and the live diagnostic path print on `stderr`; the final assistant answer is the only normal `stdout` content. Logs default under `.ainiux-pr/logs/agent/` with the same live-flush / finalize behavior as security-review logs (`tail -f` the printed `.partial` path). Each model round records normalized input, fresh-input, cache-read, cache-write, and output token counts when the provider reports usage (including OpenAI cache details and DeepSeek cache hit/miss fields); completed-turn totals fall back to local estimates when needed. Disable logs with `--no-agent-log`.

Interactive Agent reports `Agent preparing` only while tools, the project database/history, prompts, project instructions, and the cheap index probe are being prepared; index refresh and snapshot loading never gate readiness. Preparation is local and starts immediately even when the startup provider picker has temporarily selected offline `none`; it does not wait for provider or model selection. Failed required preparation reports `Agent unavailable`. Startup and direct model selection never call `/models` for context-window discovery: they use `models.conf` when available and otherwise show token-only usage. Explicit `/models` and model-picker actions still request the provider catalog. Automatic credit checks run independently with a bounded fallback timeout. During provider retry backoff, a redacted live notice is shown and removed when the retry resolves; it is never sent back in provider context.

Agent compaction supports `/compact [fast|smart|summary]`. `fast` is bounded deterministic local compaction and never calls a model; `smart` is the configured default and escalates when the local candidate would be too large or lossy; `summary` always uses the active provider/model when compactable history exists. A bare `/compact` and automatic compaction use `[agent] compact_strategy`, while an argument overrides one invocation. Automatic compaction begins at 75% of every known context window unless `compact_limit` explicitly overrides it. Setting `auto_compact = off` disables proactive and context-error recovery compaction but leaves manual `/compact` available. When building the model-visible compact projection, successful `read_file` and `read_many` results are reduced to path/status stubs (bodies can be re-read from the workspace); the summary prompt also forbids pasting those file dumps into the checkpoint. A recent unsummarized tail is kept within about 8% of the context window. The TUI immediately appends a persistent notice naming the requested strategy, shows an animated `Agent compacting` activity row, and appends a one-line success, skipped, cancelled, or failure result. Successful results report wall time plus approximate tokens saved and tokens remaining in the model-visible context; skipped results explain that no removable middle history exists. Internal checkpoint payloads are not replayed into visible history; every original transcript and full tool-event row remains preserved on disk.
...
`Ctrl+G` toggles the editor with the conversational mode that opened it:
...
Explicit `chat`, `agent`, and `editor` commands work in the editor with or
without a leading slash; chat and agent use `/chat`, `/agent`, and `/editor`.
...
The default `builtin` corpus is embedded from `benchmarks/builtin.jsonl` at build time and is also installed under `share/ainiux/benchmarks`. Results are JSONL records on `stdout`; progress, the final summary, status, and errors remain on `stderr`. Every result includes the current prompt and tags. It also carries an optional external-file URL, reference answer, assessment criteria, and safety rating when configured; harmful safety cases receive a `harmful-request` tag and sensitive cases receive `policy-sensitive`. The summary is a two-column table by default; `--summary-format csv` emits `metric,value` CSV instead. `--quiet` suppresses progress and the summary but not JSONL results or errors. If `--output` names an existing directory or ends in `/`, ainiux creates it when needed and writes a timestamped `benchmark-*.jsonl` file plus a formatted `benchmark-*.md` report with the same basename. Explicit `.jsonl` output paths receive the equivalent `.md` companion; other explicit filenames have `.md` appended. The Markdown report renders prompts, external links, correct answers, assessment criteria, provider usage, responses, errors, and the aggregate summary, with column-aligned Unicode tables for field/value and summary sections. Stdout-only runs do not create files. `--case ID`, `--category NAME`, and `--limit N` select cases. `--runs N` controls measured repetitions outside speed mode, while `--warmup N` runs separate unreported repetitions. Pressing `Ctrl+C` stops new work, cancels active HTTP requests, joins workers, writes an interrupted final summary, and exits with status 130. The opt-in long-context file fetches two Project Gutenberg works and therefore requires network access; normal built-in cases are fully local until sent to the configured model endpoint.
...
`--reasoning auto|VALUE|TOKENS`, `/reasoning [VALUE]`, and `/setting reasoning=VALUE` all use one canonical selection. `auto` omits an override; `off` resolves to a matched model's documented disabling choice (`none`, `disabled`, `0`, `no_think`, or another catalog value); a known model without one rejects `off` and lists its choices. An unmatched model retains semantic `off`, which the provider adapter encodes with its disabled wire shape. A non-negative integer is an exact token budget, and bounded ASCII values such as `low`, `max`, `none`, `enabled`, or future values pass through without approximate conversion. Bare `/reasoning` opens a model-aware selector whose choices come from `models.conf`. When a family matches but another direct value is absent from its `value` list, one-shot CLI mode warns on stderr and continues; REPL, chat TUI, and editor commands warn and ask for y/n confirmation.

Chat stores the selection per thread as JSON `null`, a string, or an integer. Changing the actual provider or model resets it to Auto, while loading or cloning a thread restores its saved value. The standalone editor remembers its last provider/model/API/reasoning selection globally in SQLite; explicit CLI arguments still win. Endpoint URLs and credentials are never stored in this app state, so a remembered custom provider is restored only while a usable endpoint remains configured. Catalog temperature metadata is advisory for explicit values: presets omit known-incompatible temperature settings, while an explicit temperature is serialized with a warning. In chat, agent, and editor modes, `Ctrl+T` advances reasoning through the model catalog's ordered values, then Auto, and cycles again. Toggle-only Qwen 3.5/3.6 and Gemma 4 models instead alternate thinking off/on through `chat_template_kwargs.enable_thinking`; with no selected model the shortcut is a silent no-op. The separate `/thinking show|hide` command and `Alt+Ctrl+T` control trace display only. Ordinary chat also binds `Ctrl+W`; the editor retains `Ctrl+W` for closing a buffer. See [docs/api-compatibility.md](docs/api-compatibility.md) for wire mappings and catalog limits.
...
The TUI keeps model requests, `/models`, `/save`, and `/load` behind runtime jobs so the terminal loop stays responsive. Its always-visible input label is `Ainiux vVERSION | Ctrl+H help | history Ctrl+B ↑ Ctrl+D ↓`, and its initial status is `Tab complete | Ctrl+Space continue | Alt+Enter newline`; after a completed streaming response, the status shows time to first token and token/s, using compact notation such as `~20.0 token/s` when provider token usage is unavailable and locally estimated. With `--context`, that same line also shows estimated context usage. Non-streaming responses show total response latency because true first-token timing is not observable. The bottom input area embeds the editor component in a fixed-height panel with soft wrap and visual-row cursor movement. In chat TUI mode, `Tab` is context sensitive: at the beginning of the first input line it completes slash commands, and after `/insert`, `/attach`, `/save`, or `/load` it completes file paths with repeated-choice cycling. Empty input and non-file commands do not start path completion. In the thread list, `Tab` / `Insert` starts a new thread instead. Colors are enabled by default with the `dark` theme; use `--nocolors` or `/theme off` to disable color styling, `/theme` or `/theme NAME` to inspect or switch themes, and `/highlight on|off` to toggle raw Markdown highlighting. GFM pipe tables in chat and agent history are display-only reformatted into column-aligned Unicode box tables (padded GFM when the locale is not UTF-8); stored transcripts keep the model’s raw Markdown, and open tables reformat live as rows stream in. Thinking traces are hidden by default; use `/thinking show`, `/thinking hide`, `Alt+Ctrl+T`, or ordinary-chat `Ctrl+W` to toggle display of `<think>...</think>` blocks; `Ctrl+T` cycles the selected model's reasoning setting instead. Visible thinking traces retain their dedicated style while surrounding Markdown is highlighted. Provider reasoning fields such as `reasoning_content`, `reasoning`, and text `reasoning_details` are displayed as `<think>` blocks. TUI chat threads are stored in `~/.ainiux/ainiux.db` using SQLite WAL mode; `/list` opens a newest-first thread picker, up/down changes selection, letter keys jump to matching labels, Enter loads a thread, Tab/Insert starts a new thread, and Esc cancels. A thread missing its saved provider or model is prefixed with `[SETUP: … missing]`; loading it immediately opens provider selection and then model selection. Sending and regeneration remain disabled until both choices are complete, and an empty saved model never inherits a model or endpoint from the previously active thread. Threads whose attachment media expired or disappeared are labeled `[RO]`; their transcript can be read, listed, saved, or removed, but no new model turn or transcript edit is allowed. `/new [NAME]` starts a fresh thread, `/provider` or `Ctrl+P` opens the provider picker (or `/provider PROVIDER` sets one directly), `/model MODEL` changes the model, `/pop` removes the last user or assistant message, `/response` replies to a final unanswered user message, and `/remove` asks before soft-deleting the current thread. `Enter` sends, `Alt+Enter` or `Esc` then `Enter` inserts a newline, `Shift` plus arrows, `PageUp`/`PageDown`, `Home`/`End`, or `Ctrl+Home`/`Ctrl+End` extend a highlighted selection in the input, `Ctrl+A` selects the entire input buffer, `Ctrl+E` copies the last user or assistant message into the input for editing (`Enter` saves, a bare `Esc` cancels), `Ctrl+C` copies the selection, Backspace/Delete on a selection cuts it, `Ctrl+H` toggles help, `Ctrl+V` pastes, `Ctrl+K` kills from the cursor to the end of the input line and removes the line when it is already empty, `Ctrl+Z` or `Ctrl+U` undoes, `Ctrl+Y` redoes, and `Ctrl+S` sends the current multiline draft. A bare `Esc` cancels the active model request while keeping the current turn visible. `Ctrl+R` regenerates the last answer by resending the last user prompt and its managed images. `/pop` removes the last user or assistant message. `Home`/`End` move to the current input line, `Ctrl+Home`/`Ctrl+End` jump to buffer bounds, `PageUp`/`PageDown` page through the input like the editor, `Ctrl+B` and `Ctrl+D` scroll chat history back and forward, and `Alt+Home`/`Alt+End` jump to the oldest history or live bottom. `Ctrl+G` cycles to the editor; `Ctrl+Q` exits chat mode.

The mouse wheel scrolls exactly one rendered visual row per event. Chat and agent scroll only while the pointer is over history or a visible help panel; prompt, status, confirmation, and picker areas ignore it. The editor scrolls only the focused pane's content without moving the caret or changing the buffer, including in help and settings views; keyboard navigation, editing, or paste resumes normal cursor following. Full-screen modes use xterm-compatible mouse reporting, so native terminal selection may require `Shift+drag`, depending on the terminal.

With an input selection, `Ctrl+C` copies that selection. With no input
selection, it copies the last user or assistant message from the stored
transcript and publishes it through the clipboard bridge. This preserves the
message's original newlines and excludes painted soft-wrap padding, labels,
and scrollbar glyphs that some full-screen terminal clients include in mouse
selections.

In chat and agent modes, `Ctrl+O` toggles the history scrollbar; the equivalent
command is `/scrollbar show|hide`. The state follows chat ↔ editor ↔ agent mode
switches. In the editor, use `Esc` and enter `/scrollbar hide` or
`/scrollbar show`; `Ctrl+O` retains its existing Open File behavior. Hiding a
scrollbar leaves its column blank and preserves the current text wrapping, so
mouse selection no longer captures the `█`/`░` scrollbar cells.
...
Markdown rendering uses terminal font attributes in both chat history and the editor: headings and strong text are bold, emphasis is italic, and links and URLs are underlined. Delimiter recognition is punctuation-aware, keeps intraword underscores such as `window_width_max` literal, and never removes blockquote or answer text. `--nocolors` or `/highlight off` keeps the corresponding plain rendering behavior.
...
v0.99 introduced `--security-review`, a headless read-only whole-project workflow on the project index and native provider function calls. v1.01 added one-shot agent mode; v1.02 enabled ordinary workspace writes; v1.03 separated user profile (`~/.ainiux/`) from project agent/index state (`.ainiux-pr/`) and added multi-turn project sessions; v1.04 added live tool streaming and chat/agent transcript isolation; v1.05 added permanent agent chrome and default-allow guarded commands; v1.06 added the clipboard bridge, responsive agent widgets, and interactive Guard Ask persistence. v1.07 added session-scoped Act/Plan task modes, one-shot `plan`/`--plan`/`--plan-file`, trusted Plan prompts, code-enforced planning-document-only writes, and safe normalization of contained absolute mutation paths. v1.08 added one-row provider reasoning previews, in-place live tool completion rows, project-specific preview limits, and complete display-notice/thinking isolation from provider resume, compaction, and token-estimate projections. v1.09 added stable Agent prompt caching/token accounting, vetted Smart read-only command approval, expanded Plan inspection commands, and live-filesystem command output for non-indexed project paths. v1.10 keeps mutation-aware incremental refresh while restoring the fast definitions-only code index, adding static symbol importance, optional startup fallback, one-shot tool failure metrics, shell-free `command -v`, and common package-query formatting in guarded commands. v1.11 adds `fast`, `smart`, and `summary` agent context compaction with persistent progress and result notices. v1.12 adds retained row-diff terminal rendering and punctuation-aware Markdown highlighting across chat, agent, and editor. v1.13 adds a first-run Yes/No code-index offer in interactive agent mode, an “index” chrome marker when enabled, a timed completion line before the index summary table, and clears the CLI index progress callback so `--index-code --print-index` cannot use a dangling printer under ASan. v1.14 adds editor text layout and line-cleanup commands, chat/agent history `/width` and align modes, and pretty tables capped to the history width with cell word-wrap. Security review remains read-only.
