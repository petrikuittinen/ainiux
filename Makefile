...
ZLIB_LIBS ?= $(shell pkg-config --libs zlib 2>/dev/null || printf '%s\n' -lz)
CXXFLAGS += $(LIBCURL_CFLAGS) $(SQLITE_CFLAGS) $(ZLIB_CFLAGS)
LDFLAGS += $(LIBCURL_LIBS) $(SQLITE_LIBS) $(ZLIB_LIBS)
...
WEB_STYLESHEET := src/web/css/app-v27.css
WEB_JAVASCRIPT := src/web/js/app-v37.js
...
$(WEB_ASSET_HEADER): $(WEB_INDEX) $(WEB_STYLESHEET) $(WEB_JAVASCRIPT) $(WEB_HIGHLIGHT_JAVASCRIPT) $(WEB_SYNTAX_JAVASCRIPT) $(WEB_IMAGE_OPTIONS_JAVASCRIPT) $(WEB_VIDEO_OPTIONS_JAVASCRIPT) $(WEB_EDITOR_HISTORY_JAVASCRIPT) $(WEB_EDITOR_INDENTATION_JAVASCRIPT) $(WEB_SELECTOR_JAVASCRIPT)
	...
	@{ \
		printf '%s\n' '#pragma once' '#include <string_view>' 'namespace ainiux::server::web {' \
			'inline constexpr std::string_view kStylesheetPath = "/ui/assets/app-v27.css";' \
			'inline constexpr std::string_view kJavascriptPath = "/ui/assets/app-v37.js";' \
