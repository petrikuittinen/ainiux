...
Implementation note (2026-07-05): `--thinking` and `--thinking-budget` now pass through a provider compatibility layer for OpenAI Chat/Responses, OpenRouter, Gemini, Qwen/DashScope, DeepSeek, xAI, and custom/local OpenAI-compatible fallback profiles. Native Anthropic extended thinking, live model capability probing, and preservation of opaque reasoning state for future agentic tool loops remain open.
