...
agent has since landed through v1.08; reconcile every item with the current
...
**Read-only tool parallelism (next):** today one agent worker runs each model tool round **sequentially** in `execute_prepared_calls` even when the provider emits a multi-call “parallel group” (used only for identical-call fingerprints and history order). Implement the §14 scheduler for **read-only** calls so a single round can fan out bounded concurrent work:
...
Other remaining agent work (updated for v1.08): security/refactor task-mode selection, `find_callers` / `find_callees`, transcript as sole TUI source of truth, bounded read-only multi-tool parallelism, and stronger editor↔agent handoff. Act/Plan selection and compact reasoning/activity previews are complete.
