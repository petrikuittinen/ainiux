...
agent has since landed through v1.09; reconcile every item with the current
baseline below before treating it as open work:

- Dedicated agent security/refactor task modes and further purpose-built agent UI.
...
Implementation status (2026-07-27): `ainiux` is at **v1.09**. The local agent is a shipped product surface with one-shot Act (`run` / `--run`) and Plan (`plan` / `--plan` / `--plan-file`) entries plus interactive `agent` / `--agent`. Session-scoped `/plan` and `/act` preserve transcript and compaction state while replacing the trusted task layer and tool policy. Interactive projects persist Confirm/Smart/Yolo permissions, show the mode in the prompt border, and isolate Agent commands from Chat/editor AI-assist commands. OpenRouter, OpenAI, and DeepSeek Agent sessions asynchronously show remaining API-key credit beside that permission mode when the selected key can access the provider's balance endpoint. Validated native exact-path tools and direct-argv commands can use approved external paths while hard Guard and Plan denials remain absolute. Plan retains read/index/research tools but code-enforces planning-document-only writes. Interactive agent rounds show bounded, redacted provider-supplied reasoning previews, and live tool completion replaces the running row in place. Display-only `notice` and `thinking` rows persist for history but are excluded from provider resume, compaction, and token-estimate projections. Continue this track with measurement-gated read-only tool parallelism, call-graph tools, agent-transcript/TUI decoupling, stronger editor handoff, and later security/refactor task modes.

Historical snapshot (2026-07-24): v1.06 had already landed the project-local multi-turn agent runtime, compact/live tool activity, transcript-preserving compaction, `/new`, Guard Ask approvals, and trusted master+coding prompts. v1.07 subsequently completed the Act/Plan slice described in the current baseline.

Implementation note (2026-07-26, v1.08): provider-native tool rounds expose only readable reasoning text from Chat Completions `reasoning_content`, textual `reasoning`/`reasoning_details`, `<think>` traces, and Responses reasoning summaries/text. Encrypted or opaque reasoning remains protocol continuation state and is never displayed. Interactive agent history receives one structured `thinking` row per successful readable round and stable-key tool rows whose completion replaces the running row. Previews normalize whitespace, redact configured credentials, clip by grapheme count and terminal width, and persist only the final bounded value. `[tui] agent_thinking_preview_max_chars` supplies the 0–1000 global limit; agent-only `/setting thinking_preview_max_chars=N` persists a project override. One-shot agent stdout/stderr behavior is unchanged. Durable `notice` and `thinking` rows are display-only and excluded from provider resume, compaction summaries/projections, and transcript estimates; intentional retry and protocol guidance continues as explicit model-conversation messages.
...
Implementation note (2026-07-18, corrected 2026-07-26): interactive startup follows the same model-discovery rule as an in-session provider change. Chat and Agent require an LLM: when no provider is known they open the shared provider selector, then perform cancellable `/models` discovery; when the provider is known but the model is not, they start directly with model discovery. Exactly one discovered model is auto-selected and several open the shared model selector. A supplied model never substitutes for a missing provider. Plain Editor startup remains a usable non-AI editor with the offline provider and no modal setup; selecting an online provider later enables the same discovery flow for optional AI assistance.

Implementation note (2026-07-26): layered `[model]` records may provide an optional `context_window` fallback when a provider's `/models` response identifies a model but omits its context length. Explicit `--context`/`/context` values remain authoritative, followed by usable endpoint metadata, then the catalog fallback. The bundled DeepSeek V4 Pro/Flash family records the provider-documented 1M window. The bundled catalog is embedded at build time so the fallback remains available when an uninstalled binary is launched outside the source directory; development/installed files and system/user layers retain their documented override order.
...
**Agent git/index/web/task tools slice (2026-07-22):** registry gains `git_status`, `git_diff` (read-only git CLI with hardening; `git diff` allowlisted in process policy), `index_status`, `index_update` (optional path filter / force rescan), agent-only `index_rebuild` (`confirm=true`), heuristic `find_tests` and `inspect_code_task`, and agent-only network tools `fetch_url` / `search_web` reusing `src/fetch` and `src/search` (private URL policy from CLI/config). Security-review stays mutation- and network-free. Still missing from the §12 catalog: `find_callers`, `find_callees` (need refs/call-graph). The then-future Guard Ask UI and Plan mode subsequently landed; security/refactor modes remain later work.

**Guard Ask UI + DB persistence slice (2026-07-22):** interactive agent TUI prompts y/n for Guard `Ask` decisions via `ApprovalGate` (worker blocks; UI resolves). Outcomes persist in `.ainiux-pr/agent.sqlite` `approvals` plus transcript `notice` lines. Headless `run` still maps Ask→Deny. Hard Deny rules remain non-elevatable. Agent git policy expands (add/commit/push/reset/clean/…) so user-approved Ask forms can actually execute; `remove` database deletes also go through Ask. Plan mode later landed in v1.07; security/refactor task modes and call-graph tools remain later work.

**Interactive agent project reset + manual compaction slice (2026-07-23):** `/new [PATH]` resolves relative paths from the active project, expands only `~`/`~/`, creates at most the final missing component, rejects `.ainiux-pr` symlinks, and asks default-No before replacing existing project state. The warm runtime releases old handles, initializes and switches workspaces without leaving agent mode, and restores the prior project on initialization failure; non-agent files are untouched. `/compact` uses the automatic compaction pipeline as a cancellable TUI job, bypasses the percentage threshold, atomically persists a summary event, preserves the full transcript, and replaces only model-visible request context. Repeated compaction without enough new history is a no-op.

**Trusted prompt layering (2026-07-23):** agent sessions use master + coding + static native/XML protocol appendix; security-review keeps master + security (stricter adversarial trust posture). Soft trust boundary lives in master so AGENTS.md remains advisory project context. `plan_prompt.md` / `refactor_prompt.md` remain for future task-mode selection.

**Agent cache/token observability slice (2026-07-26):** agent sessions now use one bounded `agent_prompt.md` plus a stable native/XML appendix. Act/Plan changes append Ainiux mode controls (and changed framed root `AGENTS.md` context) instead of rewriting the earlier request prefix; compaction rebuilds only the active control. Native Act/Plan definitions are identical and ordered, with `MutationPolicy` retaining non-elevatable Plan enforcement. Provider usage normalizes OpenAI cache read/write details and DeepSeek hit/miss tokens into per-turn input/fresh/cache-read/cache-write/output totals recorded in `SessionTurnResult` and agent logs. Security review retains the byte-stable master+security composition. Batch/multi-call fixtures now record the deterministic model-round and request-byte reduction. A read-only executor pool remains gated on local tool execution reaching 5% of representative turn wall time.

**Smart read-only command approval slice (2026-07-27):** complete argv forms now pass through a conservative read-only classifier. Smart skips its ordinary `run_command` prompt only for vetted inspection/passive snapshot commands whose cwd and every path input canonically remain inside the project; Confirm and Yolo retain their respective always-ask and validated-no-prompt behavior. Plan uses the expanded classifier as a non-elevatable command policy, while security-review keeps its historical narrow snapshot allowlist. Unknown flags, external paths, symlink/protected-path escapes, output/follow/exec forms, shell syntax, and privilege escalation do not gain the exemption.

### Next agent slices (ordered intent)

**Read-only tool parallelism (measurement-gated):** today one agent worker runs each model tool round **sequentially** in `execute_prepared_calls` even when the provider emits a multi-call “parallel group” (used only for identical-call fingerprints and history order). Revisit the §14 scheduler only if telemetry shows local tool execution consumes at least 5% of representative turn wall time. If that gate is met, a single round may fan out bounded concurrent work:

- **Parallel-safe (bounded pool):** `read_file`, `read_many`, `get_skeleton`, `list_directory`, `glob`, `search_text` / `grep` / `find`, `search_symbol`, `read_symbol`, `project_overview`, `inspect_code_task`, `find_tests`, `git_status`, `git_diff`, and similar pure reads over the snapshot/index.
- **Network-limited parallel:** `fetch_url` / `search_web` under a small cap (settings `parallel.max_network_tools`, default on the order of 3) so multi-URL research does not stampede.
- **Keep serial for now:** all mutations (`edit_file`, `write_file` / `create_file`, `str_replace`, `apply_patch`, `remove`), index writes (`index_update` / `index_rebuild`), Guard `Ask`/`Deny` paths, and `run_command` (or only allow parallel for clearly read-only inspection commands after a conservative classifier). Prefer a single workspace mutation lane rather than per-path locks in the first cut.
- **Correctness requirements:** preserve provider tool-call order in history/results; honor cancellation mid-batch; no shared mutable snapshot races; unit tests for “N parallel reads succeed” and “mutation still serial / same-file serialize.” Do not invent concurrent writes until read-only parallelism is stable.

Other remaining agent work (updated for v1.09): security/refactor task-mode selection, `find_callers` / `find_callees`, transcript as sole TUI source of truth, measurement-gated read-only multi-tool parallelism, and stronger editor↔agent handoff. Act/Plan selection and compact reasoning/activity previews are complete.
...
**Status (2026-07-23):** design target. Current agent runtime executes every prepared tool call in a model round **one-by-one on the single agent worker** (`execute_prepared_calls`). Multi-call assistant turns are still accepted and ordered, but they are not concurrent. Security-review batch workers are a separate pipeline. The next agent slice should implement the rules below for **read-only** tools first; keep mutations serial (see “Next agent slices” above).

The scheduler accepts multiple tool calls from the model and executes safe independent calls concurrently when their safety class allows it.
