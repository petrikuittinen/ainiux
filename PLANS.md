...
agent has since landed through v1.07; reconcile every item with the current
...
Implementation status (2026-07-25): `ainiux` is at **v1.07**. The local agent is a shipped product surface with one-shot Act (`run` / `--run`) and Plan (`plan` / `--plan` / `--plan-file`) entries plus interactive `agent` / `--agent`. Session-scoped `/plan` and `/act` preserve transcript and compaction state while replacing the trusted task layer and tool policy. Plan retains read/index/research tools but code-enforces planning-document-only writes; contained absolute mutation paths normalize to project-relative paths before the same containment and mode checks. Continue this track with bounded read-only tool parallelism, call-graph tools, agent-transcript/TUI decoupling, stronger editor handoff, and later security/refactor task modes.

Historical snapshot (2026-07-24): v1.06 had already landed the project-local multi-turn agent runtime, compact/live tool activity, transcript-preserving compaction, `/new`, Guard Ask approvals, and trusted master+coding prompts. The v1.07 status above supersedes its then-open Plan-mode wording.
...
Other remaining agent work (updated for v1.07): security/refactor task-mode selection, `find_callers` / `find_callees`, transcript as sole TUI source of truth, bounded read-only multi-tool parallelism, and stronger editor↔agent handoff. Act/Plan selection is complete.
