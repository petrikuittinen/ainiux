...
Implementation note (2026-07-05): `--thinking` and `--thinking-budget` now pass through a provider compatibility layer for OpenAI Chat/Responses, OpenRouter, Gemini, Anthropic Claude through its OpenAI-compatible endpoint, Kimi K2.x, Qwen/DashScope, DeepSeek V4, GLM-5.2/Z.AI, xAI, and custom/local OpenAI-compatible fallback profiles. Native Anthropic Messages support, live model capability probing, and preservation of opaque reasoning state for future agentic tool loops remain open.
