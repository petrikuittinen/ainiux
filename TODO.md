...
2. DOCX↔Markdown is implemented for the supported Markdown-native subset, including `/chat-to-docx`, `/last-to-docx`, and the WebUI download of that normalized transcript. Recurring headers/footers, footnotes/endnotes/comments, macros/templates, media extraction/insertion, and lossless package editing remain deferred.
...
PDF↔Markdown is implemented (`--input file.pdf`, `--output-format pdf`, `--attach file.pdf`, `--fetch-url` of `application/pdf`, editor PDF→sibling `.md`, and `/chat-to-pdf` / `/last-to-pdf`). Local and fetched DOCX conversions use canonical Markdown and never treat package or media bytes as prompt text. Local and fetched XLSX conversions use canonical Markdown tables. PPTX conversion preserves the supported slide/text/table/notes/image subset and uses placeholders in conversation/browser paths. Transcript-to-XLSX/PPTX remains unimplemented. Transcript-to-DOCX is `/chat-to-docx` and `/last-to-docx`. Legacy `.xls` and `.ppt` are rejected. Local CSV and JSON (`.csv`, `.json`) are native UTF-8 attachments across CLI, TUI, REPL, and WebUI; `--fetch-url` of CSV/JSON converts to Markdown. `.jsonl`/`.tsv` and CSV/JSON writers remain unimplemented.
