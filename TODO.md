...
PDF↔Markdown is implemented (`--input file.pdf`, `--output-format pdf`, `--attach file.pdf`, `--fetch-url` of `application/pdf`, editor PDF→sibling `.md`, and `/chat-to-pdf` / `/last-to-pdf`). Local and fetched DOCX conversions use canonical Markdown and never treat package or media bytes as prompt text. Local and fetched XLSX conversions use canonical Markdown tables (`--input file.xlsx`, `--output-format xlsx`, `--attach`, `--fetch-url`, editor open/save, WebUI attach and workspace `.xlsx` save). Transcript-to-XLSX remains unimplemented. Legacy `.xls` is rejected. Local CSV and JSON (`.csv`, `.json`) are native UTF-8 attachments across CLI, TUI, REPL, and WebUI; `--fetch-url` of CSV/JSON converts to Markdown. `.jsonl`/`.tsv` and CSV/JSON writers remain unimplemented.
