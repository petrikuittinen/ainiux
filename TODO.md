...
- Agent client `web_search` returns at most **3** results; hosted provider `web_search` (catalog `web_search=on`) takes precedence when the current adapter can emit it. GPT-5 / Grok 4 / DeepSeek V4 (including Vision) stay on client search under Chat Completions, including after a live `/provider` or `/model` switch. Native Gemini generateContent/Interactions and native Anthropic Messages search remain future work.
...
- Keep explicit JSON export/import compatibility, not the primary local chat library.
...
2. DOCX↔Markdown is implemented for the supported Markdown-native subset, including TUI `/export`, `/export-last`, and the WebUI download of that normalized transcript. Recurring headers/footers, footnotes/endnotes/comments, macros/templates, media extraction/insertion, and lossless package editing remain deferred.
3. PPTX↔Markdown is implemented across CLI, fetch/chat, editor, control API, and WebUI workspace flows. Chart/SmartArt semantics, animation/themes, multi-turn image editing, transcript-to-PPTX, dedicated browser downloads, macros/encryption, and lossless package edits remain deferred. Keep future additions behind `src/pptx/` and the shared bounded OOXML boundary.

PDF↔Markdown is implemented (`--input file.pdf`, `--output-format pdf`, `--attach file.pdf`, `--fetch-url` of `application/pdf`, editor PDF→sibling `.md`, and chat export). Local and fetched DOCX conversions use canonical Markdown and never treat package or media bytes as prompt text. Local and fetched XLSX conversions use canonical Markdown tables. PPTX conversion preserves the supported slide/text/table/notes/image subset and uses placeholders in conversation/browser paths. TUI latest-message export supports multi-table XLSX and one-table CSV; whole-transcript XLSX and transcript PPTX remain unimplemented. Legacy `.xls` and `.ppt` are rejected. Local CSV and JSON (`.csv`, `.json`) are native UTF-8 attachments across CLI, TUI, REPL, and WebUI; `--fetch-url` of CSV/JSON converts to Markdown. `.jsonl` and `.tsv` writers remain unimplemented.
