...
Image request formatting supports multiple input images. In `auto` mode, the client combines registry-level provider capability with a matched `models.conf` `images` flag when present, then recognized vision-model names. Catalog `images = off` keeps a family text-to-text (DeepSeek V4 Pro and legacy `deepseek-v4-flash`, GLM-5.3); `images = on` marks text-image-to-text (official DeepSeek `deepseek-flash` / `deepseek-v4.1-flash*`, `deepseek-v4-flash-vision-exp`, GLM-5.3-Flash, Qwen3.8-Flash-Next). Unknown compatible models require `--image-capability allow`, while `deny` disables image input. This is conservative client-side detection rather than a live provider capability protocol; endpoints can still reject formats their active model cannot decode.

## Image generation API

`ainiux image` selects a compiled protocol from `images.conf`. `openai_images` posts JSON to `{base}/images/generations` or `{base}/images/edits` (when `--attach` PNG/JPEG files are present). The OpenAI default model is `gpt-image-2`. GPT Image models always return `data[0].b64_json`; this slice does not download OpenAI `url` results or stream partial images. OpenAI may require organization verification.

`replicate_predictions` posts `{ "input": { ... } }` to `{base}/models/{owner}/{name}/predictions` with `Prefer: wait=60`, polls `urls.get` if needed, cancels on interrupt, and downloads the first output file URL (Authorization required). Image `--attach` files are `data:` URLs in the catalog `images_field`. Video attachments are uploaded first through `{base}/files` (multipart `content`) and the returned HTTPS file URL is placed in the catalog start/end or reference fields. Safety inputs are the most permissive field each model documents (`disable_safety_checker`, `enable_safety_checker: false`, `disable_safety_filter: true`, `safety_tolerance: 5`, or `safety_filter_level: block_only_high`). Replicate credentials are `REPLICATE_API_KEY` then `REPLICATE_API_TOKEN`; `AINIUX_API_KEY` is not used. Video records live in `videos.conf` with `protocol = replicate_predictions`.

`fal_queue` posts the catalog input object to `{base}/{endpoint_id}` (`https://queue.fal.run/fal-ai/flux/schnell`) with `Authorization: Key …`, polls `status_url` until `COMPLETED`, cancels with `PUT cancel_url`, then downloads `images[0].url`. fal credentials are `FAL_API_KEY` then `FAL_KEY`. Safety uses `enable_safety_checker: false` or the highest documented `safety_tolerance`. WIDTHxHEIGHT becomes fal’s `{width,height}` object when `size_field` is `image_size`. This path is independent of Chat Completions vs Responses (`--api`). Other OpenAI Images, Replicate, or fal models can be added as `[image]` records without new C++.

`xai_imagine` reuses `--provider xai` (`https://api.x.ai/v1`). Image generation posts JSON to `{base}/images/generations` with `response_format: b64_json`, `resolution` (`1k`/`2k`), `aspect_ratio`, and `quality`. Edits post to `{base}/images/edits`: one `--attach` becomes `image.url` as a data URI; two to five become `images[]`. Video generation posts `{model, prompt, duration, aspect_ratio, resolution}` to `{base}/videos/generations`, then polls `{base}/videos/{request_id}` until `done`, `failed`, or `expired`. Image-to-video adds `image.url`; reference-to-video adds `reference_images`. Credentials are `XAI_API_KEY` then `GROK_API_KEY` then `AINIUX_API_KEY`.

Gemini video generation reuses `--provider gemini`. `gemini_interactions` video records (`gemini-omni-1.1-flash`) post `{model, input, response_format:{type:video,delivery:uri}, store:true}` to `{native_base}/interactions` and parse `steps` for a video `uri` or `data`. URI delivery requires `store: true`. `gemini_veo` posts `{instances, parameters}` to `{native_base}/models/{model}:predictLongRunning`, polls the returned operation until `done`, and downloads `generateVideoResponse.generatedSamples[0].video.uri` with `x-goog-api-key`. Image-to-video uses `image` plus optional `lastFrame`; reference-to-video uses `referenceImages` with `referenceType: asset`. Mode suffixes in `-m` are stripped before the wire model. Credentials are `GEMINI_API_KEY` then `AINIUX_API_KEY`.

`gemini_interactions` image generation posts `{model, input, response_format, store:false}` to `{native_base}/interactions` with `x-goog-api-key` (not Bearer). The existing `--provider gemini` chat base `https://generativelanguage.googleapis.com/v1beta/openai` is rewritten by stripping `/openai`. `--size` tokens are sent as uppercase `K` (`1K`, `0.5K`) in `response_format.image_size`; `--ar` is `aspect_ratio`; `--format` becomes `response_format.mime_type`. Consumer Gemini image models default to `image/jpeg`; `gemini-3.1-flash-lite-image` accepts only JPEG (`image/png` returns HTTP 400). `--attach` PNG/JPEG files become extra `input` `{type:image, mime_type, data}` parts. REST responses are parsed from `steps` (`model_output` image blocks; thought images ignored); SDK-only `output_image.data` is a fallback. Credentials are the Gemini chat keys `GEMINI_API_KEY` then `AINIUX_API_KEY`. `safety_settings` is omitted: the consumer Gemini API rejects it (`The parameter 'safety_settings' is not available on the Gemini API`). Classic Imagen `:predict` is not implemented. Bundled Gemini models are `gemini-3.1-flash-image` (default), `gemini-3.1-flash-lite-image`, `gemini-3-pro-image`, and `gemini-2.5-flash-image`.

CLI examples and the bundled OpenAI / Replicate / fal / Gemini / xAI Imagine `-m` lists are in [CLI and scripting](cli.md#image-generation).

## Responses API

Official `--provider openai` now defaults to Responses (`POST https://api.openai.com/v1/responses`). Keep Chat Completions with `--api chat`, `openai_chat`, or a user `api = chat` setting. `--api responses`, `--responses`, and `openai_responses` still select Responses explicitly. Custom base URLs and `custom_openai_chat` stay on Chat Completions unless the user asks for Responses. Switching to a chat-only profile such as Gemini, Anthropic, or OpenRouter uses Chat Completions even if the previous provider or project session left `api=responses`. An explicit `--api responses` on a chat-only profile still returns `AINIUX_ERR_UNSUPPORTED_FEATURE` unless the user supplies `--responses-url URL`. xAI and DeepSeek advertise a built-in `/responses` path but default to Chat Completions; catalog `web_search = on` does not auto-select Responses. Official OpenAI is the only profile that defaults to Responses. Streamed DeepSeek Responses requests have been observed to stall on SSE `: keep-alive` comments with no output. `--api responses` still selects Responses explicitly on profiles that advertise it.

A line break or `\`-newline wrap inside `OPENAI_API_KEY` is not a Responses schema failure. libcurl/nghttp2 refuses that `Authorization` header on HTTP/2 (`CURLE_BAD_FUNCTION_ARGUMENT` / `Failed sending HTTP POST request`) while HTTP/1.1 may still send it and then get `401 Incorrect API key`. Ainiux unwraps those keys before building the request.
...
anthropic_effort, VALUE     output_config: { effort: VALUE }
anthropic_effort, TOKENS    output_config: { task_budget: { type: tokens, total: TOKENS } }
...
Catalog entries currently cover GPT-6 Astra/Sol/Luna, model-specific GPT-5 generations, Gemini/Gemma, Claude token budgets, Claude Opus 5/5.5 effort and task-budget request fields, Grok, DeepSeek V4 text on third-party routes (`deepseek-v4-pro` and dated `deepseek-v4-flash`), official DeepSeek API `deepseek-flash`, `deepseek-v4-pro`, and legacy `deepseek-v4-flash` (served as V4.1 Flash, text-image-to-text), `deepseek-v4-flash-vision-exp`, and `deepseek-v4.1-flash-expires-on-0910`, Kimi, GLM-5/5.2 (thinking still disableable), GLM-5.3, GLM-5.3-Flash, and GLM-5.3-FlashX (always-on thinking with `reasoning_effort` `low|high|max`, default `max`; Flash and FlashX are text-image-to-text), Qwen 3.5/3.6 Chat/Responses, Qwen 3.8 Chat effort, Qwen3.8-Flash-Next (multimodal; `none|low|medium|xhigh`, default `xhigh`; 256K context fallback; thinking vs instruct sampling presets), MiniMax Chat/Responses, MiMo V2.5 and V2.6 Chat/Responses (text-image-to-text; V2.6-Distill-Qwen-9B uses the Qwen thinking toggle), Stepfun Step 3.7 Flash and Step 5 Preview (Step 5 is text-image-to-text), Nemotron, Hy3, Hy4 preview, Llama 3.x presets, and both 20B/120B gpt-oss variants. Model matching checks only the final component, so arbitrarily nested prefixes such as `gateway/vendor/GEMINI-...` work without becoming part of the family expression. Anthropic [task budgets](https://platform.claude.com/docs/en/build-with-claude/task-budgets) are beta and require the documented beta header. Native Anthropic Messages is still not implemented; `anthropic_effort` only supplies request-side fields to endpoints that accept that pass-through, and cannot add native response semantics by itself.
...
GLM-5 / 5.3 / Flash web_search      { "type": "web_search", "web_search": { "enable": true } }  Chat
```

Gemini official grounding is native `google_search` on generateContent/Interactions. The official OpenAI-compat Chat adapter rejects `{ "type": "google_search" }` with HTTP 400, so Ainiux does not attach that hosted tool and keeps the client `web_search` path. GPT-5 and Grok 4 host search only on Responses; a live Chat Completions session keeps the client `web_search` function instead of sending a hosted type that the adapter rejects. DeepSeek's native web search is exposed on its Anthropic-compatible API; its OpenAI Chat schema has no hosted search type, and its Responses API ignores built-in tools. Because native Anthropic Messages is not implemented, Ainiux keeps client search for DeepSeek on both current adapters rather than sending an invalid or ignored tool. Gemini streamed `tool_calls` often omit the OpenAI `index` field and may include `extra_content.google.thought_signature`; Ainiux assigns a stable index and echoes those extras on the next turn. Anthropic hosted search on native Messages (`web_search_20250305`) is not implemented; client search remains the reliable Claude path. Hosted `web_search_call` output items are replayed and not executed locally. Citations from `url_citation` annotations are collected for display.
