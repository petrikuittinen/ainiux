...
`replicate_predictions` posts `{ "input": { ... } }` to `{base}/models/{owner}/{name}/predictions` with `Prefer: wait=60`, polls `urls.get` if needed, cancels on interrupt, and downloads the first output file URL (Authorization required). `--attach` images are `data:` URLs in the catalog `images_field`. Safety inputs are the most permissive field each model documents (`disable_safety_checker`, `enable_safety_checker: false`, `safety_tolerance: 5`, or `safety_filter_level: block_only_high`). Replicate credentials are `REPLICATE_API_KEY` then `REPLICATE_API_TOKEN`; `AINIUX_API_KEY` is not used.
