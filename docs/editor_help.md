...
Enter with **`F4`**, **`Ctrl+X d`**, CLI `ainiux -d` / `ainiux --dired [PATH]`, or `dired` / `/dired [path|glob]` in the command minibuffer (TAB completes paths). Dired is full-screen. **`q`** returns to the editor; **`Ctrl+Q`** quits ainiux. **Enter** opens a read-only preview or enters a directory; **Enter** again leaves the preview for the list. Dirty files inside a project may highlight changed lines against `.ainiux-pr/history` (same syntax colors, mild background). **`←`/`→`** go to parent / enter directory. **`o`** opens a file for edit and leaves dired. Sort with **`s`** then `(n)ame` / `(s)ize` / `(d)ate` (or uppercase for descending). **`p`** toggles the selected file dirty ↔ reviewed (content-hash; “pass”). In list or view, **Space** / **`b`** page down/up like less; in a viewed file, **`f`** or **`/`** finds. Linux rows show mode, owner, and group.
