...
WebUI chat file input follows the image-generation upload pattern: opaque IDs in a memory-only `ChatInputStore`, not workspace paths. HTML, PDF, and DOCX convert to Markdown at upload time; DOCX returns additive bounded warnings and its raw bytes are discarded. Text attachments use the existing SQLite/media import path so they are not inlined into the 1 MiB message content cap.
