...
`--fetch-url`, `/fetch`, `/attach URL`, `/insert URL`, and agent `fetch` accept `application/pdf` (and `application/x-pdf`) plus `application/vnd.openxmlformats-officedocument.wordprocessingml.document`, `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`, `text/csv`, and `application/json`. A missing or `application/octet-stream` type is sniffed for a `%PDF-` header, DOCX OPC/Word ZIP structure (`word/`), or XLSX workbook ZIP structure (`xl/`); `application/zip` is accepted only when that Word or workbook structure is present. Octet-stream CSV/JSON uses the URL path ending. A document-looking URL that returns HTML is still HTML. PDF, DOCX, XLSX, CSV, and JSON bytes are never placed directly into prompt context: each is converted locally to bounded canonical Markdown. DOCX and XLSX conversion warnings propagate to CLI and interactive chat callers. The download cap remains `--max-fetch-bytes`; its default is 10 MiB, matching the local document-input default while retaining an explicit bound for untrusted responses.

Opening a PDF, DOCX, or XLSX in the standalone or WebUI editor converts it to Markdown and retargets Save to the sibling `.md`. Saving a standalone buffer explicitly to `.pdf`, `.docx`, or `.xlsx` runs the corresponding writer atomically. Original binary documents are never overwritten with Markdown bytes. WebUI save/create of a `.xlsx` path runs the XLSX writer; WebUI DOCX export remains deferred.
...
WebUI chat file input follows the image-generation upload pattern: opaque IDs in a memory-only `ChatInputStore`, not workspace paths. HTML, PDF, DOCX, and XLSX convert to Markdown at upload time; DOCX/XLSX return additive bounded warnings and their raw bytes are discarded. CSV and JSON uploads stay native text. Text attachments use the existing SQLite/media import path so they are not inlined into the 1 MiB message content cap.
