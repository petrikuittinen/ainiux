...
Image **protocols** (HTTP conversation shapes) are compiled C++. Image **models** live in `images.conf`, analogous to `models.conf` reasoning protocols. Adding another OpenAI Images or Replicate model is a catalog record (`api_model`, field names, `defaults_json`). `replicate_predictions` is implemented; `fal_queue` still needs an adapter. `models.conf` `images = on` stays vision input and is not reused for generation. The Replicate profile is image-only (`REPLICATE_API_KEY` / `REPLICATE_API_TOKEN`) and does not appear in chat provider pickers.
