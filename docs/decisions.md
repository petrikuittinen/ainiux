...
Syntax highlighting lives in `src/highlight/` and produces byte-based semantic spans without terminal escapes. `text` remains the safe fallback for scratch buffers, unknown extensions, and manual opt-out. The dependency-free engine supports Markdown, Python, C, C++, C#, Java, JavaScript/JSX, TypeScript/TSX, HTML, CSS, XML, JSON, and Bash. It keeps explicit line state for fenced blocks, comments, strings, heredocs, CDATA, and embedded HTML script/style content; resolves high-priority comments and strings before structural and word tokens; caps pathological lines; and provides an incremental document cache invalidated from before the changed line so multiline state remains correct.
