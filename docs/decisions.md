...
Model capabilities and purpose presets are likewise isolated in `models.conf` instead of growing ordinary application configuration or provider conditionals. The bundled source is embedded at build time so an uninstalled binary retains its catalog outside the repository; an available development/installed copy may replace that embedded base before the user layer is applied. Model records merge by `id`, with their ordered reasoning choices stored compactly as `value = none|low|...`, optional documented context fallback as `context_window`, and optional `max_output_tokens` ceiling metadata; presets merge by `(model_id,purpose)`, and `enabled = off` removes an earlier identity. Explicit context overrides remain authoritative, followed by usable `/models` metadata and then the catalog fallback. Output-ceiling metadata does not select a request size. Model regular expressions and closed protocol names are validated at load time. A model expression is evaluated case-insensitively against only the final slash-separated component; arbitrary routing prefixes never participate. Bundled family records are provider-neutral, while the optional provider/API scopes and priority remain available for more specific user overrides. The catalog may select only a registered provider protocol—it cannot inject arbitrary JSON paths. Qwen 3.5/3.6 Chat stays on the boolean `qwen_chat` toggle (`chat_template_kwargs.enable_thinking`). Qwen 3.8 uses a dedicated `qwen_chat_effort` protocol because its chat template accepts `reasoning_effort` (`none|low|medium|xhigh`, default `xhigh`) in addition to the same thinking on/off switch; folding that ladder into `qwen_chat` would also break the toggle-only `Ctrl+T` shortcut. Claude Opus 5 uses `anthropic_effort`: named selections become `output_config.effort`, while an exact numeric selection becomes `output_config.task_budget.total` rather than the legacy per-response `thinking.budget_tokens` field.
...
Hosted web search is a catalog capability (`web_search = on`, optional `web_search_name`), not another client search provider. When the matched model can emit the family's hosted shape on the active adapter, that tool takes precedence over Tavily/Firecrawl/Exa/Searxng/DuckDuckGo even if those are configured. Agent mode then omits the client `web_search` function. `--no-builtin-web-search` and user `web_search.builtin = off` restore the client path. Kimi `$web_search` is a builtin-function echo: the client returns the model's arguments unchanged. GLM uses a request-level `{type:web_search, web_search:{enable:true}}` wrapper. Claude hosted names are sent on the existing OpenAI-compat Chat adapter. Official Gemini OpenAI-compat Chat rejects `type=google_search`, so that family stays on client search. DeepSeek exposes its native web search on the Anthropic-compatible API, while OpenAI Chat has no hosted type and Responses ignores built-ins; the current adapters therefore retain client search. Native Gemini generateContent/Interactions and native Anthropic Messages remain unimplemented.
...
PDF conversion is canonical **PDF ↔ Markdown**. `src/pdf/` reimplements pdfio-style COS parsing and writing in C++17 (no vendored pdfio) and links **zlib** only for Flate. Plaintext and HTML are not separate PDF writers/readers: `--output-format plaintext` and `html` run `markdown::render` on the Markdown result, so HTML→PDF is HTML→Markdown→PDF and PDF→HTML is PDF→Markdown→HTML. The writer follows the pdfio `md2pdf` layout (universal 210 mm × 11 in page, Core-14 Helvetica/Courier including BoldOblique, WinAnsi, Flate content streams, classic xref) but builds the file in memory so stdout and atomic `--output` need no `/Length` seek-patching. Body text uses 1 inch margins on a MediaBox-sized page. Markdown tables measure column widths from the same Helvetica/Courier glyph metrics used to draw, wrap cell text to those inner widths (splitting an overlong word by character when it cannot fit), and size each row to the wrapped line count so cells do not paint into neighbors. A body line of space follows each paragraph and each table so headers are not tight against preceding prose. CropBox equals MediaBox (never an inset art box) and each content stream starts with a white page fill so PDFium/Edge ink-bbox matches the paper. A `---` thematic break is a hairline, not a page break; PDF extract joins pages with a blank line so PDF→MD→PDF reflows. Content operators are coalesced per same-style line before Flate — zlib is already used; the size win is fewer `Tm`/`Tj` ops, not a second compressor. CJK, Hebrew, and Arabic Markdown→PDF embed a **subset** of a system or `--font` TrueType glyf face (Type0 / Identity-H / ToUnicode), not a vendored Noto file and not CFF/OpenType collections. Latin stays Core-14 Helvetica. Arabic is mapped to Presentation Forms-B with a small joining table (no HarfBuzz); Hebrew is reversed and right-aligned. Emoji color fonts, CFF CJK, and encrypted PDFs remain later. Passwords never appear in errors.
...
Image **protocols** (HTTP conversation shapes) are compiled C++. Image **models** live in `images.conf`, analogous to `models.conf` reasoning protocols. Adding another OpenAI Images, Replicate, fal, Gemini Interactions, or xAI Imagine model is a catalog record (`api_model`, field names, `defaults_json`). `openai_images`, `replicate_predictions`, `fal_queue`, `gemini_interactions`, and `xai_imagine` are implemented. `models.conf` `images = on` stays vision input and is not reused for generation. The Replicate and fal profiles are image-only (`REPLICATE_API_KEY` / `REPLICATE_API_TOKEN`, `FAL_API_KEY` / `FAL_KEY`) and do not appear in chat provider pickers. fal uses `Authorization: Key`, not Bearer. Gemini image generation reuses `--provider gemini` (`GEMINI_API_KEY`) and posts to the Interactions API with `x-goog-api-key`, not the OpenAI-compat chat base. Native Grok Imagine reuses `--provider xai` (`XAI_API_KEY` / `GROK_API_KEY`) rather than a fourth image-only profile.
...
Repeatable `--attach PATH` and interactive `/attach PATH` reuse the same case-insensitive document classifier as `--input`. Each local text or PDF attachment is read incrementally with a default 10 MiB `--max-input-bytes` cap, checked for binary NUL bytes (text/HTML), and validated as UTF-8. PDF bytes are converted to Markdown instead of being inserted as binary. Markdown is the canonical durable representation: plaintext is already valid Markdown, Markdown is retained, and HTML is converted exactly once in the attachment worker. The original path is provenance, not a replay dependency.
...
XLSX is handled by `src/xlsx/` using the same zlib ZIP/XML helpers as DOCX. The reader streams `sheetData` rather than building a DOM, interned shared strings, and number-format metadata only for date detection. The writer emits a compact new workbook from GitHub-flavored Markdown tables. This is conversion, not lossless spreadsheet editing: drawings, charts, comments, pivots, macros, encryption, and Excel layout are omitted. Malformed or encrypted packages fail without partial Markdown. Speed is a first-class constraint: typical small workbooks convert in well under 50 ms on this machine.

PPTX is handled by `src/pptx/` over the shared `src/ooxml/` facade to the hardened zlib ZIP and namespace-aware XML foundation. The reader resolves the OPC root and follows `p:sldIdLst` relationships rather than filenames, lazily reading slide/notes/table relationship parts and inflating image parts only for explicit file-sidecar or direct PPTX round trips. Limits cover archive members, slides, XML depth/bytes, selected data, and aggregate shapes/cells; ZIP64 reads, CRCs, cancellation, and fatal-output rollback remain shared behavior. The deterministic writer emits one reviewed master, blank layout, theme, stable relationships, fixed timestamps, level-6 XML deflate, stored compressed images, and PowerPoint autofit. Canonical Markdown uses exact top-level `---` lines as slide boundaries and a final speaker-notes quote block. Themes, charts, SmartArt, animation, exact layout, and lossless package editing are deliberately excluded. A Markdown file output may publish exact content-addressed media sidecars atomically; every conversation/editor-open/browser path emits placeholders and retains no extracted image bytes.

DOCX is handled by the dependency-free `src/docx/` boundary using the already-linked zlib. Its bounded ZIP/OPC/WordprocessingML reader inflates only the main document, relationships, styles, and conditional numbering parts; it never extracts media. Both DOCX input and output pass through canonical Markdown, as do compound PDF/HTML conversions. This deliberately produces normalized documents instead of losslessly updating an OOXML package. Images become visible placeholders, unsupported story parts are omitted with category-deduplicated warnings, and font family/size/color, alignment, and ordinary indentation normalize away. Ordinary and complex-script bold/italic properties remain distinct, adjacent equivalent Word runs coalesce before Markdown emphasis is emitted, and tab or edge whitespace stays outside emphasis delimiters. The writer emits a deterministic compact package with stable ordering, timestamps, relationships, styles, and level-6 raw DEFLATE. No XML, ZIP, office-suite, Python-runtime, or WebUI framework dependency is added.

PDF input converts to Markdown in `src/pdf/` rather than inserting binary prompt text. Page `/Contents` accepts a direct or indirect stream or array of streams, resolving array members in order under the existing object and decoded-size bounds. Markdown-to-PDF writing uses the same module (`pdf::from_markdown`) with `--output-format pdf`.

`--fetch-url`, `/fetch`, `/attach URL`, `/insert URL`, and agent `fetch` accept PDF, official DOCX/XLSX/PPTX MIME types, CSV, and JSON. A missing, octet-stream, or generic ZIP type is sniffed using bounded signatures and validated OPC root/main relationships so unrelated ZIPs and other Office families are not misclassified. Octet-stream CSV/JSON uses the URL path ending. A document-looking URL that returns HTML is still HTML. Structured bytes are never placed directly into prompt context: each is converted locally to bounded canonical Markdown, with PPTX media left uninflated. Conversion warnings propagate to CLI and interactive callers. The download cap remains `--max-fetch-bytes`; its default is 10 MiB, matching local document input while retaining an explicit bound for untrusted responses.
...
Chat export is normalized by the shared transcript layer. TUI `/export` and `/export-last` select JSON, PDF, DOCX, or Markdown; latest-message export also converts all Markdown tables to XLSX or the sole table to RFC 4180 CSV. Images and attachments become named placeholders in rendered formats. Assistant `<think>` blocks and thinking-role rows are omitted there, while JSON preserves stored content. TUI export runs as cancellable file work, uses private atomic writes for JSON and shared atomic writes otherwise, rejects link/non-regular targets, and fingerprints confirmed overwrite targets before replacement. The WebUI exposes JSON, PDF, DOCX, and Markdown; its legacy PDF/DOCX slash commands remain compatible. REPL commands are unchanged.
...
WebUI `/fetch URL` and the Fetch button call `POST /ainiux/v1/chat/inputs/fetch` and queue the result as one of those chat inputs. They do not insert a user message the way TUI `/fetch` does. HTML, PDF, DOCX, XLSX, and PPTX become Markdown; CSV and JSON stay native UTF-8 text; PNG, JPEG, and GIF are stored only when the named model accepts images. Private, loopback, link-local, multicast, and metadata addresses stay refused unless the server was started with the explicit override. A failed fetch leaves other queued inputs in place. The download still uses the existing fetch timeouts and `--max-fetch-bytes` cap.
...
Browser-oriented startup is a major mode entered through `ainiux -w`,
`ainiux --webui`, or `ainiux webserver` (`ainiux server --webui` remains a
long form). It reuses the same listener/router, uses the current directory when
`--workspace` is omitted, selects wildcard
...
Native Grok Imagine is a compiled `xai_imagine` protocol on the existing `xai`
chat profile, not a fourth image-only provider. Image generation is synchronous
JSON to `/v1/images/generations` and `/v1/images/edits` with `b64_json`. Video
generation is async `request_id` polling on `/v1/videos/generations`. Text,
image-to-video, and reference-to-video stay three catalog records because a
