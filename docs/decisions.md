...
v0.77 adds the first editor AI feature: continue/auto-write on `Ctrl+Space`. Its current design keeps prefix-only prose continuation for text and Markdown, while every structured syntax mode performs language-aware gap completion with bounded prefix and postfix context. Context is sliced by UTF-8 characters (invalid bytes are preserved as one unit), and zero disables a context side. A separate code stream filter removes only optional matching/unlabeled fences and otherwise preserves generated bytes exactly. Continue runs on cancellable runtime jobs with one undo record; cancellation keeps partial output. SSE completion is detected through `finish_reason` and `[DONE]`, then the HTTP stream is aborted so the editor can return to idle without a keypress. For `lmstudio`, `ollama`, `vllm`, and loopback custom base URLs, editor startup auto-selects the first `/v1/models` entry when `--model` is omitted.
