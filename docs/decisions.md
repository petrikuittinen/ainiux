...
## Shared syntax highlighting foundation (Markdown preview)

Syntax highlighting lives in `src/highlight/` and produces byte-based semantic spans without terminal escapes. The first enabled language is Markdown; `text` remains the safe fallback for scratch buffers, unknown extensions, and manual opt-out. The engine keeps explicit line state for fenced blocks and HTML comments, resolves higher-priority comments/code/structure before inline markup, caps pathological lines, and provides an incremental document cache invalidated from before the changed line so multiline state and setext headings remain correct.
...
The initial UI exposes `/highlight` in editor/chat and per-buffer `/mode auto|text|markdown|md` in the editor. The process-wide toggle is shared across editor/chat switches but is not persisted. Other language modes and tagged fenced-code delegation are deliberately deferred until the Markdown slice has been exercised interactively.
