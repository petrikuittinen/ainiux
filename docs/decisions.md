...
Image input is currently non-interactive and Chat Completions-only. The default limit is 20 MiB per image and can be lowered with `--max-image-bytes`. Base64 data is temporary request state: it is cleared after the provider call and never written into chat JSON. Provider/model image capability detection remains future work, so an endpoint may still reject or ignore a format accepted by the client.
