...
The follow-up URL safety layer checks the actual IPv4/IPv6 address passed to libcurl's socket-open callback and refuses private, loopback, link-local, multicast, and metadata ranges before creating the socket. Proxy URL fetching requires `--allow-private-url-fetch` because proxy-side target DNS cannot be verified locally. Charset conversion, allow/block domain configuration, and runtime-job integration for TUI/web fetches remain follow-up work.
...
Image input is currently non-interactive and Chat Completions-only. Repeated `--attach` accepts multiple images alongside text context. The default limit is 20 MiB per image and can be lowered with `--max-image-bytes`. Base64 data is temporary request state: it is cleared after the provider call and never written into chat JSON. Conservative capability detection combines provider-registry support with recognized vision-model names; `--image-capability allow` is the explicit override for a verified compatible custom model.
...
The same text loader now backs TUI `/insert PATH` in a runtime file job. It checks cancellation between bounded read chunks and delivers converted content to the owning terminal loop through the event queue; the worker never mutates the chat session directly.
