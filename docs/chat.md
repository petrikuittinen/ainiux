...
Text, Markdown, HTML, PDF, DOCX, and XLSX are converted into bounded canonical Markdown. Local CSV and JSON attachments remain native UTF-8 text. DOCX image placeholders remain visible and conversion warnings appear in status; raw package/media bytes are not retained as chat text. PNG, JPEG, and GIF attachments require a compatible Chat Completions model. Managed media cleanup never turns missing content into silent empty context; affected threads are marked read-only.
