...
Core commands include `/help`, `/new`, `/list`, `/edit`, `/provider`, `/model`, `/system`, `/setting`, `/clone`, `/save`, `/load`, `/remove`, `/pop`, `/response`, `/insert`, `/attach`, `/fetch`, `/chat-to-pdf`, `/last-to-pdf`, `/chat-to-docx`, `/last-to-docx`, `/search`, `/theme`, `/scrollbar`, and `/thinking`.
...
`/insert` places text into the input. `/attach` adds provider context or a supported image; DOCX, XLSX, PPTX, PDF, and HTML are converted to Markdown. Local CSV and JSON stay native text. Raw Office/PDF bytes are never submitted as prompt text. `/fetch`, `/attach URL`, and `/insert URL` accept HTML, PDF, DOCX, XLSX, PPTX, CSV, and JSON; structured documents become Markdown locally. PPTX images are visible omission placeholders and are neither extracted nor persisted. `/chat-to-pdf [PATH]` writes the thread to PDF (`chat.pdf` by default). `/last-to-pdf [PATH]` writes the last message (`last.pdf`). `/chat-to-docx [PATH]` and `/last-to-docx [PATH]` write the same transcript as a normalized Word file (`chat.docx` and `last.docx`). Images stay `[image: name]` placeholders. A converted attachment is named (`Attached: report.pdf`) and its Markdown body is left out; the saved chat and the model request still include that text. Thinking traces are omitted, so the assistant section is the reply text. Existing files are not overwritten. In the WebUI, one Export dialog on a thread or on the latest assistant message downloads that same PDF, DOCX, or the `/save` JSON file, and Import creates a thread from a `/load` JSON file. Transcript-to-XLSX/PPTX commands remain deferred. `/search` is an explicit network operation. A URL typed in ordinary prompt text is not fetched. In the WebUI, `/fetch URL` and the Fetch button queue that page as an attachment for the next message. HTML, PDF, and Office pages are stored as Markdown; CSV and JSON stay text; images are queued only when the thread model accepts them. A failed fetch leaves other queued files in place. Private addresses are refused unless the server was started with the explicit override. TUI `/fetch` still inserts the fetched text as a user message immediately.
