...
`models.conf` contains repeatable `[model]` and `[preset]` records. Model matching uses validated case-insensitive regular expressions against the final slash-separated model component. Records can describe API, context window, documented `max_output_tokens` capability metadata, reasoning protocol and choices, temperature support, optional `temperature_max` (default 1.0; Gemini families use 2.0), priority, optional hosted `web_search` / `web_search_name`, and optional `images` (`on` = text-image-to-text, `off` = text-to-text; omitted records keep name-heuristic detection). `max_output_tokens` records a ceiling and does not make Ainiux request that many tokens automatically. Purpose presets remain a CLI/catalog feature (`--purpose`) and are not shown in the interactive settings widget.
...
WebUI Video tab. `protocol` selects `fal_queue`, `replicate_predictions`, `xai_imagine`, `gemini_interactions`, or `gemini_veo`.
...
| `gemini` | `gemini_interactions` / `gemini_veo` | `gemini-3.1-flash-image` (images) / `gemini-omni-1.1-flash` (video) |
