...
| Unreleased | WebUI Chat/Agent prose gains dependency-free safe Markdown rendering with semantic headings, responsive tables, HTTP(S) links, and TUI-derived light/dark colors. Its first fenced-code batch highlights JavaScript, TypeScript, Python, C, C++, HTML (including embedded CSS/JS), CSS, and Bash. Native Windows x64 parity and interactive/TUI image generation remain later work. |
