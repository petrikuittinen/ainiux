...
| v1.30 | Complete control server: authenticated asynchronous jobs and SSE replay, MCP 2026-07-28 adapter, interactive agent/Guard sessions, revision-safe chat/workspace/editor operations, optional TLS/direct access, and an embedded responsive vanilla-JavaScript light/dark browser controller; the browser-startup amendment adds managed authentication, persistent browser login, reconnect, URL reporting, and best-effort browser launch |
| v1.31 | WebUI/Agent Markdown now renders semantic headings, responsive `<table>` elements, underlined clickable HTTP(S) links, and TUI-derived light/dark colors. Fenced code has full TUI language parity across Markdown, Python, C/C++, C#, Java, JavaScript/TypeScript, HTML/XML, CSS, JSON, Bash, PHP, Perl, Ruby, Rust, Go, PowerShell, Assembly, SQL, TOML, YAML, and INI, including aliases, multiline constructs, and CSS/JavaScript embedded in HTML. Rendering is dependency-free, DOM-only, inert for raw HTML, bounded, and covered by focused/unit/control-server tests; immutable asset URLs were versioned for cache safety. |
| v1.32 | The WebUI gains TUI-style model pickers with Alt+P/Alt+M shortcuts and compact Settings-linked toolbars, immediate per-thread request settings, shared persistent agent/editor model configuration, idle agent history/Act–Plan restoration, and bounded editor undo/redo. Agent activity and workspace dired/file views receive TUI-derived colors and syntax highlighting. The Image tab adds catalog-driven generation controls, ordered bounded reference uploads, and reset behavior. The browser editor adds native-parity per-file indentation detection and controls, Tab/Shift+Tab block editing, and adaptive language-aware selection/file reformatting. Native Windows x64 parity and REPL/TUI image generation remain later work. |
| v1.33 | Headless CLI, control API, and WebUI video generation land through layered `videos.conf` with `fal_queue`, `replicate_predictions`, `xai_imagine`, `gemini_interactions`, and `gemini_veo`. Native Grok Imagine image/video and Gemini Omni Flash / Veo 3.1 video reuse the existing `xai` and `gemini` profiles. DeepSeek `deepseek-v4.1-flash-expires-on-0910` is cataloged as text-image-to-text like `deepseek-v4-flash-vision*`. REPL/TUI image and video jobs remain later work. |
| v1.34 | PDF support arrives with fast local PDF-to-Markdown and Markdown-to-PDF conversion. The C++17 PDF reader and Markdown writer are hardened for hostile, malformed, and oversized input with bounded recursive/object/page traversal, checked numeric and stream ranges, stable object storage, complete Flate validation, and deterministic text geometry; focused adversarial, sanitizer, and corpus performance coverage keeps the conversion reliable and exceptionally fast. |
| v1.35 | Fast local DOCX-to-Markdown and Markdown-to-DOCX conversion receives a reproducible eight-file benchmark against `python-docx` and Spire.Doc, including fresh-process speed, independent canonical-semantic scoring, package validation, and LibreOffice cross-checks. Embedded image preservation is not supported yet: DOCX images become visible placeholders and generated DOCX files do not embed Markdown images. |
| v1.36 | Local XLSX↔Markdown conversion turns worksheet cell values into GitHub-flavored tables and Markdown tables into new workbooks. CSV and JSON are supported as native UTF-8 inputs, while fetched CSV becomes a Markdown table and fetched JSON becomes a fenced, pretty-printed JSON block. XLSX conversion preserves cell data rather than workbook layout and rich features such as charts, drawings, pivots, and macros. |
| v1.37 | Native dependency-free PPTX↔Markdown conversion adds ordered slides, presentation dimensions, rich text, lists, links, tables, speaker notes, exact content-addressed image sidecars, deterministic output, fetch/chat/editor/control-API/WebUI integration, hostile-package limits, a generated fixture corpus, structural linting, and a reproducible performance/interoperability harness. Conversion is normalized: charts, SmartArt, animation, themes, and exact layout are not reproduced. |
| v1.38 | The WebUI becomes a first-class major mode through equivalent `-w`, standalone `--webui`, and `webserver` entry points, with the current directory as the default workspace. Browser mutations gain an ephemeral in-memory CSRF token and inert raw-HTML regression coverage. The model catalog adds GPT-6 Astra/Sol/Luna and Claude Opus 5.x capabilities, including provider-specific Anthropic effort and task-budget request mapping. |

## Current status after v1.38

The current product includes CLI chat, PDF/DOCX/XLSX/PPTX conversion through canonical Markdown, local CSV and JSON inputs, fetched CSV/JSON conversion, common legacy charsets, REPL, saved-thread TUI chat, standalone editor with dired, benchmarks, judge grading, security review, one-shot and interactive agents, session Act/Plan policy, Guard approvals, `/goal`, transcript-preserving compaction, a lightweight definitions index, mid-turn editor/dired review while an agent turn continues, and an MCP client for agent/run/plan (HTTP + stdio). Apple Silicon macOS source builds are supported. Native Windows implementation is present but remains unreleased until the UCRT64 parity gate in [Native Windows](windows.md) passes.

The control-API server is complete through PR 10: one-shot jobs, MCP, interactive sessions, workspace review and revision-safe mutations/editor assist, revision-safe chat threads, TLS/direct non-loopback policy, and the embedded responsive browser controller are implemented. Catalog-driven CLI/WebUI image and video generation are shipped. Local PDF, DOCX, XLSX, and PPTX conversion is implemented through canonical Markdown; HTML compound conversion uses the same boundary. DOCX embedded-image preservation remains unimplemented, while PPTX file conversion can publish exact content-addressed media sidecars. The OpenAI-compatible `/v1` proxy remains a later adapter. Interactive/TUI image or video generation, `/loop`, sub-agents, and the native Anthropic Messages adapter are also not implemented. See [PLANS.md](../PLANS.md) for the remaining roadmap.
