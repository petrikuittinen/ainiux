...
| Version | Main result |
| --- | --- |
| Unreleased | WebUI TUI-style model pickers with Alt+P/Alt+M shortcuts and compact Settings-linked toolbars, immediate per-thread request settings, shared persistent agent/editor model configuration, idle agent history/Act–Plan restoration, and bounded editor undo/redo for keyboard, paste, and AI edits |
...
| Unreleased | Agent thinking, tool calls, notices, approvals, and errors now have distinct TUI-derived WebUI colors, and fenced activity excerpts are highlighted. Workspace dired colors directories/executables, while its file viewer and live editor detect TUI languages by path and refresh highlighting as drafts change. The Image tab now derives dependent provider/model/size/aspect/quality/format controls from layered `images.conf`, supports constrained custom dimensions, sends ordered multiple PNG/JPEG references through authenticated bounded memory-only uploads with opaque expiring IDs, and can reset the prompt, references, and generation settings while retaining provider/model selection. Native Windows x64 parity and REPL/TUI image generation remain later work. |

## Current status after v1.31
