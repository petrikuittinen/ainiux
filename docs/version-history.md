...
Unreleased work after v1.35 includes local CSV and JSON input (`.csv`, `.json`) as native UTF-8 text on `--input`, `--attach`, TUI/REPL `/attach`, and WebUI chat upload, with `--fetch-url` converting CSV to a Markdown table and JSON to a fenced pretty-printed code block.

## Current status after v1.35
