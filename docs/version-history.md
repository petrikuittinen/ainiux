...
The control-API server is complete through PR 10: one-shot jobs, MCP, interactive sessions, workspace review and revision-safe mutations/editor assist, revision-safe chat threads, TLS/direct non-loopback policy, and the embedded responsive browser controller are implemented. Catalog-driven CLI/WebUI image and video generation are shipped. CLI PDF↔Markdown conversion (`--input file.pdf` and `--output-format pdf`) is implemented and designed for super-fast local conversion; HTML-to-PDF is HTML→Markdown→PDF. The OpenAI-compatible `/v1` proxy remains a later adapter. Interactive/TUI image or video generation, DOCX conversion, `/loop`, sub-agents, and the native Anthropic Messages adapter are also not implemented. See [PLANS.md](../PLANS.md) for the remaining roadmap.
