...
| Unreleased | Native Windows 10 1903+/Windows 11 x64 UCRT64 target remains gated on native parity. CLI image generation is landed. v1.3 PR 7 adds revision-safe remote chat-thread listing/loading/creation/message appends on top of the PR 6 interactive-agent and read-only-workspace surface. |
...
The loopback control-API server, one-shot jobs through PR 3, the MCP adapter in PR 4, interactive sessions in PR 5, read-only workspace review/dired/file routes in PR 6, and revision-safe chat threads in PR 7 are implemented. Filesystem mutations, TLS/non-loopback mode, OpenAI-compatible `/v1` proxy, and browser UI remain later slices. Interactive/TUI image generation, PDF/DOCX conversion, `/loop`, sub-agents, and the native Anthropic Messages adapter are also not implemented. See [PLANS.md](../PLANS.md) for the ordered server work.
