...
| v1.19 | Interactive `/setting` widget in chat, agent, and editor (grouped rows, catalog `temperature_max`, staged save); purpose removed from the user-facing setting surface; `goal_met` advertised only while `/goal` is Active; built-in conversion of UTF-16, Windows-1250/1251/1252, ISO-8859-1/2, and KOI8-R/U to UTF-8; `--encoding`; editor encoding picker; CJK names via allowlisted `iconv` |
