...
PDF input is converted to Markdown locally; raw PDF bytes are not sent as prompt text. Markdown-to-PDF writing is local (Core-14 / WinAnsi, no network, images as alt text only). DOCX is not read as text or uploaded.
