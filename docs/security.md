...
- Agent `fetch` **always** returns UTF-8 **Markdown or plain text** (HTML→MD via `src/html/`; PDF/DOCX→MD via their bounded local converters). It never returns raw HTML/CSS/JS or binary document bytes to the model: full pages are a prompt-injection and token-cost hazard, while package bytes are not prompt text. CLI `--fetch-url` may still export HTML for local use.
...
- content type: accepts empty content type, `text/html`, `application/xhtml+xml`, `application/pdf`, the official DOCX type, the official XLSX type, and (for text fetch) `text/plain`; generic bodies with a `%PDF-` header, DOCX OPC/Word structure, or XLSX workbook structure are recognized, while `application/zip` is accepted only with Word (`word/`) or workbook (`xl/`) parts
