...
components, special files, and missing files are rejected. Ordinary editor files
are limited to 1 MiB. Raw workspace DOCX/XLSX/PPTX may be up to 20 MiB, but
their converted Markdown is still limited to 1 MiB. Directory listings and
recursive review are bounded. `.ainiux-pr`,
...
Interactive command permissions are layered after structural parsing, the destructive-command Guard, and command-aware canonical path validation. Confirm asks for every otherwise executable `run`. Smart automatically approves complete argv forms in the built-in read-only inspection/passive-snapshot set, plus in-project `mkdir`/`rmdir`/`rm`/`mv` when classified; `rm -r` of a non-empty directory still asks. Unknown flags, non-vetted commands, and external paths ask. Yolo skips the ordinary permission prompt after validation. Hard Guard denials, protected metadata, traversal, unsafe symlinks, malformed structure, and Plan-policy violations cannot be approved in any mode.
...
Agent `run` children get a minimal environment (trusted PATH, locale, pager/Git/rg overrides, and `HOME`). API keys are **not** inherited by arbitrary commands. When the resolved executable is this Ainiux process (nested `ainiux image`, `ainiux --list-models`, …), the child also receives the parent’s provider credential environment variables and `XDG_CONFIG_HOME` / `XDG_CONFIG_DIRS` so image generation and other nested CLI modes can use the same keys the user already set. Workspace copies of `ainiux` do not match the running binary and stay keyless.
...
- `apply_patch`, index/symbol/search tools, and native filesystem tools remain project-scoped. Validated exact-path native filesystem tools may access external paths according to the active interactive permission mode; external changes create no project history or index entry.
- Git inspection uses `run git …` (pager/external-diff disabled in the process runner).
...
- Agent `fetch` **always** returns UTF-8 **Markdown or plain text** (HTML→MD via `src/html/`; PDF/OOXML→MD via bounded local converters). It never returns raw HTML/CSS/JS or binary document bytes to the model: full pages are a prompt-injection and token-cost hazard, while package bytes are not prompt text. CLI `--fetch-url` may still export HTML for local use.
- Path escape, `~user/…`, `$ENV` components, `.ainiux-pr` / `.ainiux` / `.git` components, and symlink components are refused for ordinary workspace writes. Absolute and `~/…` paths are expanded (HOME) and compared to the expanded workspace; if they stay under the project they are rewritten to project-relative paths, otherwise they stay the interactive external-write exception. On POSIX a mid-path `~` component is still refused so `src/~/x` cannot create `$workspace/src/~/x`. After resolve, every ordinary write path is re-checked to stay under the project workspace root. The narrow exception is interactive Act-mode `write`: Ainiux resolves and displays the exact external target, blocks the worker for a one-shot Yes/No decision, and writes only after Yes. No/Esc, cancellation, headless operation, and Plan mode deny it. Approved external writes do not create `.ainiux-pr/history` backups or update the project index.
- `read`, `ls`, `grep`, and project-script `cwd` use the same expand-then-compare rule, so `/home/eye/project` and `~/project` are in-project when that directory is the active workspace. `read` still uses one-shot approval for an exact outside-project regular file. The usual UTF-8, size, line-range, output cap, and configured-secret redaction checks still apply. Other index tools remain workspace-contained.
...
Security-review never enables mutation or network tools and never injects project `AGENTS.md` as instructions. Agent `run` is still **argv-only** (no real shell, no unquoted `|`/`&`/`;` chaining). Workspace scripts are first-class: `./server.sh start`, bare `server.sh` under the project cwd/root, `bash server.sh …` / `sh ./script.sh`, and reusable helpers at `scripts/ainiux/NAME`. Smart and Yolo execute that last form without asking; Confirm asks once per path and content hash (shown as path, args, size, and a short hash — not the file bytes; Review opens the script in dired) and reuses that trust across interpreter and path-form invocations until the file changes. Script-hash trust lives in `.ainiux-pr/agent.sqlite` and survives quit; deleting `.ainiux-pr` clears it. Free-form shell code (`bash -c` / `sh -c` / `nohup`) remains denied in Confirm/Smart. Multi-line or wrapping `python3 -c` is also hard-denied there; a short one-liner may still Ask. Long-running Act commands may set `background=true`; the process is tracked and killed when the agent session finishes. Yolo still elevates hard Guard denials at the user's risk. Interactive user `/shell` / `!` is a separate UI feature (see above). Final assistant text is written to `stdout`; status, notices, and errors go to `stderr`. Turn/loop limits and transport retries follow the agent-loop reliability rules (identical-call soft/hard caps, consecutive-failure abort, 50-turn scripted cap, no automatic tool re-execution).

### Interactive permission modes and Guard approvals

Each project persists one interactive Agent permission mode in `settings_json`: `confirm`, `smart` (the legacy/fresh-project default), or `yolo`. Confirm asks for native writes, all exact-path native access outside the project, and every otherwise executable model-issued command. Smart allows project writes and native access beneath the canonical system temporary directory, asks for other external native access, automatically runs vetted project-contained read-only commands and classified in-project `mkdir`/`rmdir`/`rm`/`mv`, and asks for other model-issued commands. `rm -r` of a non-empty directory asks in Smart. Yolo skips elevatable prompts after complete validation **and** skips hard Guard denials (shells, sudo, package managers, host control, …) at the user's risk; it still never runs a real shell for unquoted control operators. Database/destructive actions ask in Smart and are allowed in Yolo. One tool call produces at most one consolidated prompt.
...
- **Interactive** `ainiux agent` / `--agent`: the tool worker blocks and the TUI shows a dedicated Guard approval panel that starts at the top of the request. Arrows, Page Up/Down, Home/End, and the mouse wheel scroll a long prompt. The user presses **y** (allow once), **n**/Esc (deny), or **r** Review when the Ask names a workspace script (opens that file in dired; **q** returns to this dialog without deciding). Job cancel also cancels a pending Ask. Ordinary Guard Ask approvals are **one-shot** (not sticky across later tools). Confirm trust for an unchanged `scripts/ainiux/` helper is the exception: that path+hash is remembered in the project store.
...
- content type: accepts empty content type, HTML, PDF, the official DOCX/XLSX/PPTX types, CSV, JSON, and (for text fetch) plaintext; generic bodies with a PDF header or validated OPC root/main relationship are recognized, while generic ZIP bodies are accepted only when they identify Word, workbook, or presentation parts
...
Image input is explicit through `--input IMAGE` or repeated `--attach IMAGE` combined with a prompt. Supported endings are matched case-insensitively and file signatures must match PNG, JPEG, or GIF before data is sent. The default 20 MiB input cap limits both binary reads and subsequent base64 growth; use `--max-image-bytes N` to lower it for constrained environments. **That cap is not a provider context guarantee** — base64 expands ~4/3 and some APIs account for data-URL text aggressively. Prefer smaller sources, or the local vision MCP bridge which can downscale to a long edge of ~1024 via optional Pillow/ffmpeg (`scripts/image_mcp_server.py`). WebP input is disabled in the ainiux core attach path because tested vision endpoints did not handle it reliably; the Python vision bridge may still open WebP when Pillow or ffmpeg is available.
...
`videos.conf` protocol. Reference bytes are signature-checked and bounded
before upload. Billable submissions disable ambiguous POST retries.

FAL uploads references to one-hour CDN objects. FAL credentials are sent only
to FAL API hosts; signed CDN upload URLs and generated-video download URLs
receive no authorization header, must use HTTPS, and retain resolved-address
...
Replicate video jobs upload attachments through `{base}/files` and poll
`{base}/models/{owner}/{name}/predictions`. The Replicate bearer token is sent
...
The default `--image-capability auto` mode requires both a provider profile whose adapter can carry image parts and either a catalog `images = on` match or a recognized vision-model name. Catalog `images = off` keeps text-only families (for example legacy DeepSeek V4 Flash, `deepseek-v4-flash`) from sending pixels. Official DeepSeek API `deepseek-flash` is cataloged as text-image-to-text. `--image-capability allow` is an explicit trust decision for compatible unknown/custom models; it does not make an incompatible provider understand images.

## Text Attachments

`--attach PATH` and interactive `/attach PATH_OR_URL` may send selected contents to the configured model endpoint. Full-screen chat converts text-like, PDF, DOCX, XLSX, and PPTX attachments once to canonical Markdown. PPTX embedded images become visible placeholders; raw package/image bytes are not retained. Local CSV and JSON stay native UTF-8 text. Up to `[media] max_size_to_store_to_db` UTF-8 bytes are stored directly in SQLite; larger Markdown is copied into the private content-addressed media directory as a `.md` object. Moving or changing the original source does not alter saved replay content. Inline Markdown does not expire automatically; file-backed Markdown follows the same cleanup, tombstone, and read-only-thread protections as managed images. Interactive `/insert FILE_OR_URL` instead places bounded converted text in the current editor buffer or chat draft and is not sent anywhere until the user later sends that chat draft. URL insertion and attachment are explicit network operations: only HTTP(S) is accepted, and the normal private-address/DNS/proxy/TLS/timeout/size protections apply. Remote PDF, DOCX, XLSX, PPTX, CSV, and JSON are converted to Markdown locally; HTML becomes Markdown by default, while disabling `input.auto-convert-html-to-md` preserves untrusted raw HTML in the draft. Inserted chat text and attachment context can appear in saved transcripts.

PDF, DOCX, XLSX, and PPTX input are converted to Markdown locally; raw document bytes are not sent as prompt text. Local CSV and JSON are sent as UTF-8 text; fetched CSV/JSON become Markdown. Markdown-to-PDF writing is local (Core-14 / WinAnsi plus an optional subsetted system/`--font` TrueType for CJK, Hebrew, and Arabic, no network, images as alt text only). Shared OOXML ZIP/XML parsing is bounded, validates CRCs, rejects encrypted, traversing, duplicate, overlapping, unsupported, expansion-bomb, or malformed archives, and inflates only selected metadata/XML plus explicitly requested PPTX images. PPTX placeholder conversion never inflates media. CLI file output may atomically publish exact content-addressed image sidecars; stdout/chat/TUI/editor-open/browser conversion never does. Markdown-to-PPTX reads only local allowlisted image formats, never fetches HTTP/data URLs, and WebUI workspace paths must remain inside the fixed workspace. Browser uploads retain only converted Markdown and bounded diagnostics. Legacy `.xls`/`.ppt` and macro/template/show presentation variants are rejected.
