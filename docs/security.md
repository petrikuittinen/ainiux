...
- content type: accepts empty content type, `text/html`, `application/xhtml+xml`, `application/pdf`, the official DOCX type, and (for text fetch) `text/plain`; generic bodies with a `%PDF-` header or DOCX OPC/Word structure are recognized, while `application/zip` is accepted only with that DOCX structure
