...
The vetted set includes project display/search/checksum commands (`pwd`, `ls`, `cat`, `head`, non-following `tail`, `stat`, `file`, `wc`, `du`, `grep`, `rg`, print-only `find`, `diff`, `cmp`, `readlink`, checksum tools), passive process/system snapshots (`ps`, `df`, identity/OS/load/memory/CPU display), and strict non-mutating forms of `hostname`, `date`, `ifconfig`, and `ip … show|list`. It rejects recursive symlink following, process/file-output helpers, checksum verification inputs, follow mode, command execution from `find`/`rg`, and all unrecognized option shapes. Git, interpreters, builds, tests, `ping`, and `top` are not Smart-exempt. Plan uses this same conservative classifier as a non-elevatable command allowlist; security-review retains its older, narrower index-snapshot command policy. Command execution is still direct `execve` with fixed PATH, null stdin, bounded output, and cancellation. Redirects and other shell syntax are rejected rather than parsed because accepting them would require a security-sensitive shell grammar and output-target policy. Privilege escalation (`sudo`, `doas`, `su`, and equivalents) remains hard-denied for model commands; `/shell` is the separate explicit user-typed surface.
