...
Python/C/C++ reference edges and their caller/PageRank scores are approximate navigation metadata, not an authorization source. The per-turn code-index hint is locally generated, explicitly labeled for verification, bounded by `[agent] code_index_hint_max_symbols` and `code_index_hint_max_bytes` (16 and 4 KiB by default), and removed from provider history after the turn. It is not displayed or stored in `agent.sqlite`; diagnostic agent logs may still contain the serialized provider request, as they do for all prompts. Graph-derived paths remain subject to the ordinary fingerprint, containment, permission, and live-source checks before reads or edits.

`--disable-indexing` is a strict, session-scoped Agent/Run/Plan control. Ainiux does not probe or open the project index, generate the request-only hint, run in-memory lexical rescans after writes, or schedule mutation/final refreshes. Index-backed tools and `replace_symbol` are not exposed; workspace containment, Guard approvals, exact-path live reads, ordinary edits, Git, and configured network policy remain unchanged. The option never deletes or rewrites an existing index database.
...
The vetted set includes project display/search/checksum commands (`pwd`, `ls`, `cat`, `head`, non-following `tail`, `stat`, `file`, `wc`, `du`, `grep`, `rg`, print-only `find`, `diff`, `cmp`, `readlink`, checksum tools), passive process/system snapshots (`ps`, `df`, identity/OS/load/memory/CPU display), strict non-mutating forms of `hostname`, `date`, `ifconfig`, and `ip … show|list`, and the emulated executable lookup `command -v NAME`. It rejects recursive symlink following, process/file-output helpers, checksum verification inputs, follow mode, command execution from `find`/`rg`, and all unrecognized option shapes. Git, interpreters, builds, tests, `ping`, and `top` are not Smart-exempt. Plan uses this same conservative classifier as a non-elevatable command allowlist; security-review retains its older, narrower index-snapshot command policy. Command execution is still direct `execve` with fixed PATH, null stdin, bounded output, and cancellation. Redirects and unquoted shell substitutions are rejected rather than parsed; quoted `$` package-format strings are inert argv data because no shell runs. Privilege escalation (`sudo`, `doas`, `su`, and equivalents) remains hard-denied for model commands; `/shell` is the separate explicit user-typed surface.
...
- In-memory index definitions/references are updated after a successful write so later reads in the same run stay consistent. Exact touched paths are coalesced into a cancellable persistent refresh; graph queries consume only a completed generation, and cancellation preserves the prior SQLite transaction. Tools can also call `index_update` mid-session.
...
Each project persists one interactive Agent permission mode in `settings_json`: `confirm`, `smart` (the legacy/fresh-project default), or `yolo`. Confirm asks for native writes, all exact-path native access outside the project, and every otherwise executable model-issued command. Smart allows project writes and native access beneath the canonical system temporary directory, asks for other external native access, automatically runs vetted project-contained read-only commands, and asks for other model-issued commands. Yolo skips elevatable prompts after complete validation. Recursive/database/destructive actions ask in Smart and are allowed in Yolo. One tool call produces at most one consolidated prompt.
