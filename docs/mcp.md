...
Ainiux ships a stdlib Python helper that exposes a local vision model as an MCP tool. Use it when the **agent model is text-only** (e.g. legacy DeepSeek V4 Flash, `deepseek-v4-flash`, on a local server or router) but you still want OCR/captions via a **vision-capable** OpenAI-compatible endpoint (llama.cpp, vLLM, LM Studio, …). Official DeepSeek API `deepseek-flash` and `deepseek-v4-flash-vision-exp` are multimodal and do not need this bridge.
