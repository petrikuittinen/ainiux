...
std::string reduce_stub(const std::string& tool_name,
    ...
    if (tool_name == "read") {
        content << "Arguments: "
                ...
                << "\n";
    } else if (tool_name == "outline" || tool_name == "symbol") {
...
std::string reduce_digest(const std::string& tool_name,
    ...
    if (tool_name == "edit" && args.error.ok() &&
        args.value.is_object()) {
        if (const json::Value* ops = args.value.get("ops");
            ops != nullptr && ops->is_array())
            content << "Ops: " << ops->array.size() << "\n";
    }
    if (tool_name == "write") content << "Op: write\n";
    if (tool_name == "rm") content << "Op: rm\n";
    if (tool_name == "mkdir") content << "Op: mkdir\n";
    if (tool_name == "mv") content << "Op: mv\n";
...
std::string reduce_semantic(const std::string& tool_name,
    ...
    if (!ok && content.str().find("Error:") == std::string::npos)
        content << "\nError: " << bounded_error_from_result(result_json);
    if (tool_name == "run" && command_looks_irreversible_git(arguments_json))
...
bool is_explore_tool(const std::string& name) {
    return name == "grep" || name == "ls" || name == "glob" ||
           name == "index";
}
...
ToolCompactionTier tool_compaction_tier(const std::string& tool_name) {
    if (tool_name == "ls" || tool_name == "glob" || tool_name == "index")
        return ToolCompactionTier::Prune;
    if (tool_name == "read" || tool_name == "outline" ||
        tool_name == "symbol" || tool_name == "grep" ||
        tool_name == "fetch" || tool_name == "web_search")
        return ToolCompactionTier::Stub;
    if (tool_name == "edit" || tool_name == "write" ||
        tool_name == "apply_patch" || tool_name == "mv" ||
        tool_name == "rm" || tool_name == "mkdir")
        return ToolCompactionTier::Digest;
    if (tool_name == "run") return ToolCompactionTier::Semantic;
...
bool is_reloadable_file_read_tool(const std::string& tool_name) {
    // Stub-tier results can be reloaded from workspace, index, or network.
...
void pre_shrink_compaction_middle(std::vector<CompactionLogicalItem>& middle) {
    ...
    // 1) Merge consecutive read calls into a synthetic read stub.
    {
        std::vector<CompactionLogicalItem> merged;
        merged.reserve(middle.size());
        for (std::size_t i = 0; i < middle.size();) {
            if (middle[i].role == "tool" && middle[i].tool_name == "read") {
    ...
    // 3) Drop a single read immediately followed by a digest mutation on the
    // same path (the edit digest already names the file). Batched reads retain
    // no single primary path and therefore remain useful.
    {
...
CompactionKeepList harvest_compaction_keep_list(
    ...
    for (const CompactionLogicalItem& item : middle) {
        ...
        if (item.tool_name == "run") {
            ...
            if (item.tool_name == "run" && item.has_exit_status) {
