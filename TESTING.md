...
| `make test-xlsx` | XLSX units only; `build/test_runner --xlsx-case NAME` isolates one parser/writer case |
| `make test-pptx` | PPTX units, generated-fixture check, and `pptxlint.py` self-test; `build/test_runner --pptx-case NAME` isolates `round-trip`, `boundaries`, `fixtures`, `large`, or `errors` |
...
- `tests/unit/pdf/` — COS reader (tokenizer, xref, Flate, repair) against synthetic PDFs and `tests/pdf_files/`, direct and indirect page-content stream arrays, deterministic rejection of the small `tests/fixtures/pdf_errors/error-*.pdf` malformed/incompatible corpus, PDF-to-Markdown spacing/kerning units (`TJ` word gaps vs letter kerning, DeepSeek whole-word needles), and Markdown-to-PDF writer units (COS round-trip, CropBox=MediaBox, thematic hairlines, coalesced `Tj` lines, CJK/Hebrew/Arabic Identity-H subset embed, Arabic joining and simple bidi, `llm_typical.md` needles, HTML→MD→PDF, PDF→MD→PDF reflow, emoji substitution, empty input, cancellation, table column measurement with word/character wrap, multi-page rows, and a body-line gap after paragraphs and tables).
- `tests/unit/docx/` and `tests/docx_files/` — bounded ZIP32/ZIP64, OPC relationship/content-type resolution, namespace-aware XML, styles/numbering, document-order blocks, tables, links, fragmented emphasis and complex-script formatting, image/story diagnostics, deterministic Markdown→DOCX output, round-trip stability, cancellation, and malformed/adversarial archive rejection. `--docx-only` runs the group and `--docx-case NAME` isolates a case.
- `tests/unit/xlsx/` and `tests/xlsx_files/` — streaming SpreadsheetML read/write over the shared OOXML ZIP/XML helpers: GFM table round-trip, multi-sheet headings, date systems, missing `r` attributes, namespace/UTF-16/path quirks, cached formula values, sniffing so XLSX is not classified as DOCX, cancellation, malformed archive rejection, a 240-row Wikipedia country-population table (`countries-population.html` → Markdown → `.xlsx`), and a multilingual “Hello world” table covering Latin, Han, Kana, Hangul, Arabic, Hebrew, Cyrillic, Greek, Thai, Indic, Caucasian, and Ethiopic scripts. `--xlsx-only` runs the group and `--xlsx-case NAME` isolates a case.
- `tests/unit/pptx/` and `tests/pptx_files/` — deterministic PPTX↔Markdown round trips, slide delimiters/blank slides, multilingual rich text, lists, hyperlinks, tables/merges, notes, exact media extraction/deduplication, relationship order, cancellation, three supplied 143/220/124-slide decks, and malformed ZIP/OPC/XML fixtures. `tests/pptx_files/generate_fixtures.py` recreates the small dependency-free corpus; `scripts/ainiux/pptxlint.py` checks generated package structure.
- `scripts/ainiux/docx_benchmark.py` — optional eight-file, 24-process DOCX↔Markdown speed and canonical-semantic comparison against the bundled `python-docx` adapter and Spire.Doc. It is not part of `make test` and writes raw measurements/generated files under ignored `results/`. Run the dependency-free synthetic checks with `python3 scripts/ainiux/docx_benchmark.py --self-test`. For the documented corpus, first run `make -j16 optimized`, then use `python3 scripts/ainiux/docx_benchmark.py --bootstrap --output-dir results/docx-conversion-YYYY-MM-DD --report docs/docx_conversion_benchmark.md`; `--bootstrap` installs pinned `python-docx==1.2.0` and `Spire.Doc==14.8.1` into a temporary virtual environment that is deleted afterward. The benchmark permits at most eight unique `.docx`/Markdown sources, uses explicit `./ainiux`, and refuses a nonempty output directory so a prior run cannot be overwritten accidentally.
- `scripts/ainiux/xlsx_benchmark.py` — optional eight-workbook, 48-process XLSX↔Markdown speed comparison against openpyxl 3.1.5 and the PhpSpreadsheet tree at `~/agent_analysis/PhpSpreadsheet`. It is not part of `make test` and writes raw measurements/generated files under ignored `results/`. Run `python3 scripts/ainiux/xlsx_benchmark.py --self-test` for the dependency-free table parse/emit checks. For the documented corpus, first run `make -j16 optimized`, then `python3 scripts/ainiux/xlsx_benchmark.py --bootstrap --ainiux ./ainiux --phpspreadsheet ~/agent_analysis/PhpSpreadsheet --output-dir results/xlsx-conversion-YYYY-MM-DD --report docs/xlsx_conversion_benchmark.md`. `--bootstrap` installs pinned `openpyxl==3.1.5` into a temporary virtual environment and `composer install --no-dev` in the PhpSpreadsheet tree. PhpSpreadsheet needs a PHP with `ext-zip` and XML extensions (`--php` if that is not the `php` on `PATH`). The harness uses explicit `./ainiux` and refuses a nonempty output directory.
- `scripts/ainiux/pptx_benchmark.py` — optional fresh-process benchmark over the supplied 143/220/124-slide decks. PPTX→Markdown compares python-pptx 1.0.2; Markdown→PPTX also compares PptxGenJS 4.0.1 and LibreOffice through a documented Markdown→FODP adapter. It records rotated command order, median time/throughput, peak RSS, output size, slide counts, text recall, media counts, and `pptxlint` results. Run its dependency-free smoke with `--self-test`; for a measured report, build `optimized` and use `--bootstrap --output-dir results/pptx-conversion-YYYY-MM-DD --report docs/pptx_conversion_benchmark.md`. Bootstrap dependencies live only in a deleted temporary directory.
...
- `videos.conf` parsing and overlay, FAL, Replicate, xAI Imagine, Gemini Omni, and Veo endpoint matching, scalar setting and media-limit validation, request/result mapping, managed WebUI uploads, and browser catalog/input validation for `ainiux video`
...
- Control-server WUI exact asset routing, API-auth separation, no-store
